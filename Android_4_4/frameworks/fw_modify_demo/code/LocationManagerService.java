...
import android.location.IUbxInfoListener;
import android.location.LocGyroInfo;
import android.location.LocPulseInfo;
import android.location.LocAcceInfo;
...
public class LocationManagerService extends ILocationManager.Stub {
	...
	private static final int MSG_GRY_CHANGED = 1;

	private static final int MSG_ACC_CHANGED = 2;

	private static final int MSG_PULSE_CHANGED = 3;
	...
	// used internally for synchronization
    private final Object mUbxLock = new Object();
	...
	// all receivers
    private final HashMap<Object, UbxReceiver> mUbxReceivers = new HashMap<Object, UbxReceiver>();
    ...
    public void systemRunning() {
        synchronized (mLock) {
			...
			//initUBXThread();
			//mUbxHandler = new UbxWorkerHandler(BackgroundThread.get().getLooper());
    ...
    public void reportLocation(Location location, boolean passive) {
		...
		Log.w(TAG, "reportLocation location: " + location.toString());
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_LOCATION_CHANGED:
					Log.e(TAG,"LocationWorkerHandler reportLocation");
	...
	@Override
	public void requestUbxUpdates(IUbxInfoListener listener) {
		Slog.d(TAG, "enter requestUbxUpdates");
		final int pid = Binder.getCallingPid();
        final int uid = Binder.getCallingUid();
        // providers may use public location API's, need to clear identity
        long identity = Binder.clearCallingIdentity();
        try {
            ...
            synchronized (mUbxLock) {
                UbxReceiver recevier = getUbxReceiverLocked(listener, pid,uid);
            }
        } finally {
            Binder.restoreCallingIdentity(identity);
        }
	...
	/*
	 *the speed from coagentservice[CanService]
	*/

	@Override
	public void notifyUbxSpeed(LocPulseInfo speedPulseInfo){
		//Slog.d(TAG, "enter notifyUbxSpeed "  + " time = " + SystemClock.elapsedRealtime());

        //mUbxPulseHandler.removeMessages(MSG_PULSE_CHANGED, speedPulseInfo);
        //Message m = Message.obtain(mUbxPulseHandler, MSG_PULSE_CHANGED, speedPulseInfo);
        //mUbxPulseHandler.sendMessage(m);
        handlePulseChanged(speedPulseInfo);
	}

	@Override
    public void reportAcc(LocAcceInfo accelerometerInfo) {
		//Slog.d(TAG, "enter reportAcc" + " time = " + SystemClock.elapsedRealtime());

        //mUbxAccHandler.removeMessages(MSG_ACC_CHANGED, accelerometerInfo);
        //Message m = Message.obtain(mUbxAccHandler, MSG_ACC_CHANGED, accelerometerInfo);
        //mUbxAccHandler.sendMessage(m);
        handleAccChanged(accelerometerInfo);
    }

	@Override
    public void reportGyr(LocGyroInfo gryInfo) {
    	//Slog.d(TAG, "enter reportGyr" + " time = " + SystemClock.elapsedRealtime());

        //mUbxGyrHandler.removeMessages(MSG_GRY_CHANGED, gryInfo);
        //Message m = Message.obtain(mUbxGyrHandler, MSG_GRY_CHANGED, gryInfo);
        //mUbxGyrHandler.sendMessage(m);
        handleGryChanged(gryInfo);
    }
	
	/*
	 *This interface be called by GpsLocationProvider,not used now
	*/
	@Override
    public void reportPulse(LocPulseInfo speedPulseInfo) {
    	Slog.d(TAG, "enter reportPulse");
    }

	private UbxReceiver getUbxReceiverLocked(IUbxInfoListener listener, int pid, int uid) {
        IBinder binder = listener.asBinder();
        UbxReceiver receiver = mUbxReceivers.get(listener);
        if (receiver == null) {
            receiver = new UbxReceiver(listener, pid, uid);
            mUbxReceivers.put(binder, receiver);
	...
	private void removeUbxReceiverLocked(UbxReceiver receiver) {
		Slog.e(TAG, "removeUbxReceiverLocked");

        if (mUbxReceivers.remove(receiver.mKey) != null) {
            receiver.getListener().asBinder().unlinkToDeath(receiver, 0);
        }
	}

	private final class UbxReceiver implements IBinder.DeathRecipient {
        final int mUid;  // uid of receiver
        final int mPid;  // pid of receiver

        final IUbxInfoListener mUbxListener;
       
        final Object mKey;


        UbxReceiver(IUbxInfoListener listener, int pid, int uid) {
            mUbxListener = listener;
            mKey = listener.asBinder();
            mUid = uid;
            mPid = pid;
        }
        ...
        @Override
        public boolean equals(Object otherObj) {
            if (otherObj instanceof UbxReceiver) {
                return mKey.equals(((UbxReceiver)otherObj).mKey);
            }
            return false;
        }
        ...
        public IUbxInfoListener getListener() {
            if (mUbxListener != null) {
                return mUbxListener;
            }
            throw new IllegalStateException("Request for non-existent listener");
        }

        public boolean callGryChangedLocked(LocGyroInfo gryInfo) {
            try {
                synchronized (this) {
                    mUbxListener.onUbxGpsGyroscopeChanged(new LocGyroInfo(gryInfo));
                }
            } catch (RemoteException e) {
                return false;
            }
        ...
        public boolean callAccChangedLocked(LocAcceInfo accelerometerInfo) {
            try {
                synchronized (this) {
                    mUbxListener.onUbxGpsAccelerometerInfoChanged(new LocAcceInfo(accelerometerInfo));
                }
            } catch (RemoteException e) {
                return false;
            }
        ...
        public boolean callPulseChangedLocked(LocPulseInfo speedPulseInfo) {
            try {
                synchronized (this) {
                    mUbxListener.onUbxGpsSpeedTickInfoChanged(new LocPulseInfo(speedPulseInfo));
                }
            } catch (RemoteException e) {
                return false;
            }
        ...
        @Override
        public void binderDied() {
            if (D) Log.d(TAG, "UBX listener died");

            synchronized (mUbxLock) {
                removeUbxReceiverLocked(this);
            }
        }
    }


	private void handleGryChanged(LocGyroInfo gry) {
        LocGyroInfo myGry = new LocGyroInfo(gry);

        synchronized (mUbxLock) {
			ArrayList<UbxReceiver> deadReceivers = null;
			for(UbxReceiver receiver : mUbxReceivers.values()){
				if (!receiver.callGryChangedLocked(myGry)) {
                        Slog.w(TAG, "RemoteException calling onLocationChanged on " + receiver);
						
                    if (deadReceivers == null) {
	                    deadReceivers = new ArrayList<UbxReceiver>();
	                }
	                if (!deadReceivers.contains(receiver)) {
	                    deadReceivers.add(receiver);
	                }
			...
			if (deadReceivers != null) {
	            for (UbxReceiver receiver : deadReceivers) {
	                removeUbxReceiverLocked(receiver);
	            }
        	}
        }
    }

	private void handleAccChanged(LocAcceInfo acc) {
        LocAcceInfo myAcc = new LocAcceInfo(acc);

        synchronized (mUbxLock) {
			ArrayList<UbxReceiver> deadReceivers = null;
			for(UbxReceiver receiver : mUbxReceivers.values()){
				if (!receiver.callAccChangedLocked(myAcc)) {
                        Slog.w(TAG, "RemoteException calling onLocationChanged on " + receiver);
	...
	private void handlePulseChanged(LocPulseInfo speed) {
        LocPulseInfo mySpeed = new LocPulseInfo(speed);

        synchronized (mUbxLock) {
			ArrayList<UbxReceiver> deadReceivers = null;
			for(UbxReceiver receiver : mUbxReceivers.values()){
				if (!receiver.callPulseChangedLocked(mySpeed)) {
                        Slog.w(TAG, "RemoteException calling onLocationChanged on " + receiver);
