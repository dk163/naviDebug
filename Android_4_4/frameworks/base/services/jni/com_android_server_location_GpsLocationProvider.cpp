...
//[ADD-BEGIN] by kang
static jmethodID method_reportA3dInfo;
static jmethodID method_reportGyrInfo;
static jmethodID method_reportMovInfo;
...
#ifdef STB_NAVI
//[ADD-BEGIN] by kang
static void gyr_callback(GpsGyrData* gyr_data)
{
	//ALOGI("gyr_callback endter");
	JNIEnv* env = AndroidRuntime::getJNIEnv();
    env->CallVoidMethod(mCallbacksObj, method_reportGyrInfo, gyr_data->axis,
            (jfloat)gyr_data->z, (jfloat)gyr_data->x,
            (jfloat)gyr_data->y,(jfloat)gyr_data->temp,
            gyr_data->interval,  (jlong)gyr_data->ticktime);
    checkAndClearExceptionFromCallback(env, __FUNCTION__);
}

static void acc_callback(GpsAccData* acc_data)
{
	//ALOGI("acc_callback endter");
	JNIEnv* env = AndroidRuntime::getJNIEnv();
    env->CallVoidMethod(mCallbacksObj, method_reportA3dInfo, acc_data->axis,
            (jfloat)acc_data->z, (jfloat)acc_data->x,
            (jfloat)acc_data->y,
            acc_data->interval,  (jlong)acc_data->ticktime);
    checkAndClearExceptionFromCallback(env, __FUNCTION__);
}

static void mov_callback(GpsPulData* mov_data)
{
    //ALOGI("mov_callback endter");
    JNIEnv* env = AndroidRuntime::getJNIEnv();
    env->CallVoidMethod(mCallbacksObj, method_reportMovInfo, (jlong)mov_data->ticktime,
            mov_data->interval, (jfloat)mov_data->speed);
    checkAndClearExceptionFromCallback(env, __FUNCTION__);
}
...
static void android_location_GpsLocationProvider_class_init_native(JNIEnv* env, jclass clazz) {
    ...
    method_reportGeofencePauseStatus = env->GetMethodID(clazz,"reportGeofencePauseStatus",
            "(II)V");
	//[ADD-BEGIN] by kang
	method_reportA3dInfo = env->GetMethodID(clazz,"reportA3D",
            "(IFFFIJ)V");
	method_reportGyrInfo = env->GetMethodID(clazz,"reportGYR",
            "(IFFFFIJ)V");
	method_reportMovInfo = env->GetMethodID(clazz,"reportMOV",
        "(JIF)V");
	//[ADD-END]
