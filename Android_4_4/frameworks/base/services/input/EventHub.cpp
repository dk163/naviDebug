...
status_t EventHub::openDeviceLocked(const char *devicePath) {
	...
	//zhangzhenbang add for "disable usb mouse and keyboard" on 20171114
	if (strstr(device->identifier.name.string(), "Mouse") != NULL
			|| strstr(device->identifier.name.string(), "Keyboard") != NULL) {
		ALOGI("New black device: %s", device->identifier.name.string());
		delete device;
		return -1;
	}
