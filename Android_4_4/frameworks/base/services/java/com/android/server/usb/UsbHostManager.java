...
public class UsbHostManager {
    ...
    /* Called from JNI in monitorUsbHostBus() to report new USB devices */
    private void usbDeviceAdded(String ProductName, String deviceName, int vendorID, int productID,
            int deviceClass, int deviceSubclass, int deviceProtocol,
            /* array of quintuples containing id, class, subclass, protocol
               and number of endpoints for each interface */
            int[] interfaceValues,
	...
	if (ProductName == null || ProductName == ""){
		Log.d(TAG, "TSP log USB  ProductName is null or empty");
		return;
	}else{
		if (ProductName.contains("Mouse") || ProductName.contains("Keyboard")) {
			Log.d(TAG, "isBlackListed: " + ProductName);
			Intent intent = new Intent("com.chinatsp.action.input");
	...
	if (deviceName == null || deviceName == ""){
		Log.d(TAG, "TSP log USB  deviceName is null or empty");
		return;
	}else{
		if (isBlackListed(deviceName) ||
			isBlackListed(deviceClass, deviceSubclass, deviceProtocol)) {
