...
class MountService extends IMountService.Stub
    ...
    private static final boolean LOCAL_LOGD = true;	//Old: false
    private static final boolean DEBUG_UNMOUNT = true;	//Old: false
    private static final boolean DEBUG_EVENTS = true;	//Old: false
    private static final boolean DEBUG_OBB = true;	//Old: false
    ...
    private void handleSystemReady() {
        ...
        for (Map.Entry<String, String> entry : snapshot.entrySet()) {
            ...
            if (state.equals(Environment.MEDIA_UNMOUNTED)) {
				Slog.i(TAG, "handleSystemReady mount:"+path);
				if("/storage/udisk".equals(path)){
					SystemProperties.set("service.usbmount.systemready","true");
				}
                int rc = doMountVolume(path);
                if (rc != StorageResultCode.OperationSucceeded) {
                    Slog.e(TAG, String.format("Boot-time mount failed (%d)",
                            rc));
					//32893 ʵ����̬����̨���ű������֣��ļ���������U��δ�Զ�����USB����
					SystemProperties.set("service.usbmount.first","false");
                }
	...
	private final BroadcastReceiver mUsbInputReceiver = new BroadcastReceiver() {
		@Override
		public void onReceive(Context context, Intent intent) {			
			...
			View view = inflate.inflate(
					com.android.internal.R.layout.common_toast_custom, null);
			LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(
					width / 2, ViewGroup.LayoutParams.WRAP_CONTENT);

			TextView txt = (TextView) view
					.findViewById(com.android.internal.R.id.textViewToastMsg);
			txt.setText(com.android.internal.R.string.chinatsp_usb_mouse_keyboard_not_supported);
    ...
    public boolean onEvent(int code, String raw, String[] cooked) {
        ...
        if (code == VoldResponseCode.VolumeStateChange) {
            ...
            if (code == VoldResponseCode.VolumeDiskInserted) {
				...
				//zhangzhenbang add 20170814
				sendStorageIntent(Intent.ACTION_MEDIA_UNMOUNTED_COAGENT, volume, UserHandle.ALL);
            ...
            if (action != null) {
                ...
                //zhangzhenbang add for "COAGENT" 20170808 start
                if (action.equals(Intent.ACTION_MEDIA_REMOVED)) {
					sendStorageIntent(Intent.ACTION_MEDIA_UNMOUNTED, volume, UserHandle.ALL);	//The message for Changan music app.
					sendStorageIntent(Intent.ACTION_MEDIA_UNMOUNTED_COAGENT, volume, UserHandle.ALL);	//The message for Changan music app.
                	sendStorageIntent(Intent.ACTION_MEDIA_REMOVED_COAGENT, volume, UserHandle.ALL);
                }
                //zhangzhenbang add for "COAGENT" 20170808 end	
            }
        } else {
            return false;
        }
    ...
    private void notifyVolumeStateChange(String label, String path, int oldState, int newState) {
        ...
        if (newState == VolumeState.Init) {
            ...
            if (!state.equals(
                ...
                updatePublicVolumeState(volume, Environment.MEDIA_UNMOUNTED);
                //Old: action = Intent.ACTION_MEDIA_UNMOUNTED;	//zhangzhenbang disable on 20170814
            }
        ...
        if (action != null) {
    		...
    		//zhangzhenbang add for "COAGENT" 20170808 start
            Slog.d(TAG, "COAGENT action:" + action);
			
            if (action.equals(Intent.ACTION_MEDIA_MOUNTED)) {
            	sendStorageIntent(Intent.ACTION_MEDIA_MOUNTED_COAGENT, volume, UserHandle.ALL);
            	
            } else if (action.equals(Intent.ACTION_MEDIA_UNMOUNTED)) {
            	sendStorageIntent(Intent.ACTION_MEDIA_UNMOUNTED_COAGENT, volume, UserHandle.ALL);
				
            } else if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
            	sendStorageIntent(Intent.ACTION_MEDIA_EJECT_COAGENT, volume, UserHandle.ALL);
				
            } else if (action.equals(Intent.ACTION_MEDIA_SHARED)) {
            	sendStorageIntent(Intent.ACTION_MEDIA_SHARED_COAGENT, volume, UserHandle.ALL);
            }
            //zhangzhenbang add for "COAGENT" 20170808 end
        }
    }
    ...
    private int doMountVolume(String path) {
        ...
        try {
            ...
            int code = e.getCode();
			if(-1==code)
				code = VoldResponseCode.OpFailedMediaCorrupt;
            ...
            sendStorageIntent("coagent.action.usb.damage.device", volume, UserHandle.ALL);
    ...
    private void sendStorageIntent(String action, StorageVolume volume, UserHandle user) {
		if (volume.getPath().equals("/storage/udisk_dvr")) {
			action = action + "_DVR";
		}
        ...
        if(Intent.ACTION_MEDIA_MOUNTED.equals(action) && "/storage/udisk".equals(volume.getPath())
		  && "true".equals(SystemProperties.get("service.usbmount.systemready","false"))
		  && "true".equals(SystemProperties.get("service.usbmount.first","true")) ){
			 SystemProperties.set("service.usbmount.first","false");
			 intent.putExtra("firstMount",  "true");
			 Slog.i(TAG, "sendStorageIntent set firstMount true." + ",path:" + volume.getPath());
		}
		
        Slog.i(TAG, "sendStorageIntent " + intent + " to " + user + ",path:" + volume.getPath() );
    ...
    public MountService(Context context) {
        ...
        // USB input devices changes
        final IntentFilter usbInputFilter = new IntentFilter();
        usbInputFilter.addAction("com.chinatsp.action.input");
        mContext.registerReceiver(mUsbInputReceiver, usbInputFilter, null, mHandler);
