...
class ServerThread {
    ...
    public void initAndLoop() {
        ...
        // Gopise: disable some services when bootopt
        int bootopt=SystemProperties.getInt("ro.boot.bootopt", 0);
        boolean disableDummy = false;
        ...
        {
            Slog.e(TAG, "Bootopt disabled! Normal booting...");
        }

        final boolean headless = disableSystemUI;// "1".equals(SystemProperties.get("ro.config.headless", "0"));
...
*/
        ...
        //SystemProperties.set("sys.delayload", "trigger_end_start");
...
//        final NetworkTimeUpdateService networkTimeUpdaterF = networkTimeUpdater;
        ...
        final boolean delayLoadingF = delayLoading;
...
*/
        ...
        try {
            if (twilight != null) twilight.systemReady();
        } catch (Throwable e) {
            reportWtf("makin Twilight Service ready", e);
        }
        try {
            if (uiMode != null) uiMode.systemReady();
        } catch (Throwable e) {
            reportWtf("making UI Mode Service ready", e);
        }
        try {
            if (appWidget != null) appWidget.systemRunning(safeMode);
        } catch (Throwable e) {
            reportWtf("Notifying AppWidgetService running", e);
        }
        try {
            ...
            Slog.e(TAG, "BOOTOPT: Trigger end_start!");
            SystemProperties.set("sys.delayload", "trigger_end_start");
        }
        // For debug builds, log event loop stalls to dropbox for analysis.
        if (StrictMode.conditionallyEnableDebugLogging()) {
	...
	//chinatsp
    static final void enableWelcome(Context context) {
		try{
			PackageManager pm = context.getPackageManager();
			ComponentName name = new ComponentName("com.android.welcome", "com.android.welcome.DefaultActivity");
			pm.setComponentEnabledSetting(name, PackageManager.COMPONENT_ENABLED_STATE_ENABLED,
	                	PackageManager.DONT_KILL_APP);
		} catch(Exception e){
		
		}
		Slog.d("ggggg", "enableWelcome...");
    }
    ...
    static final void startSystemUi(Context context) {
        ...
        /** 启动chinatsp主服务 */
        startChinatspService(context);
    }
    
	/**
	 * 功能：启动chinatsp主服务
	 */
	static final void startChinatspService(Context context) {
		Intent intent = new Intent();
		intent.setComponent(new ComponentName("com.chinatsp.misc",
					"com.chinatsp.misc.MiscService"));
  ...
  * 启动mcuservice
  **/
static  final  void startMcuService(Context context){

    Intent intent = new Intent();
    intent.setComponent(new ComponentName("com.chinatsp.mcuservice2","com.chinatsp.mcuservice2.McuService"));     
       Slog.d(TAG, "Starting mcuservice2 service: " + intent);
            context.startServiceAsUser(intent, UserHandle.OWNER);
    }
