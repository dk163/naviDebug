...
public class ConnectivityService extends IConnectivityManager.Stub {
	...
	RouteInfo mUsb0DefaultRouteInfo;

	RouteInfo mUsb0DefaultRouteInfoQuickAdd;
    ...
    private int getPersistedNetworkPreference() {
        final ContentResolver cr = mContext.getContentResolver();
		// just configure preferred network: 1 - WIFI
        final int networkPrefSetting = 1;//Settings.Global
...
//                if (!mNetTrackers[checkType].isAvailable()) continue;
    ...
    private boolean updateRoutes(LinkProperties newLp, LinkProperties curLp,
        ...
        for (RouteInfo r : routeDiff.removed) {
			log("updateRoutes: routeDiff.removed - " + r.toString());
            if (isLinkDefault || ! r.isDefaultRoute()) {
                if (VDBG) log("updateRoutes: default remove route r=" + r);
                removeRoute(curLp, r, TO_DEFAULT_TABLE);
				// deal with wlan0 ans usb0 co-exist issue
				log("******Removing route info: " + r.toString());
				LinkAddress destination = r.getDestination();
				log("******destination: " + destination.toString());
				InetAddress address_dest = InetAddress.parseNumericAddress("0.0.0.0");
				LinkAddress linkAddress_dest = new LinkAddress(address_dest, 0);
				log("******linkAddress_dest: " + linkAddress_dest.toString());
				if (destination.equals(linkAddress_dest)) {
					if ("wlan0".equals(r.getInterface())) {
						log("******Removing wlan0 default route: " + r.toString());
						if (mUsb0DefaultRouteInfoQuickAdd != null) {
							log("******usb0 had ever set a default gateway then IMMEDIATELY let it act as the default.");
							log("******AND few secs later connectivity will set default again just IGNORE the warning.");
							synchronized (mRoutesLock) {
		                        log("******Adding " + mUsb0DefaultRouteInfoQuickAdd + " for interface " + "usb0");
		                        try {
			                        mNetd.addRoute("usb0", mUsb0DefaultRouteInfoQuickAdd);
									// notify that usb0 had been IMMEDIATELY set a default route caused by wlan0 default removed
									log("******Notify that usb0 had been IMMEDIATELY set a default route caused by wlan0 default removed.");
									mUsb0DefaultRouteInfo = mUsb0DefaultRouteInfoQuickAdd;
		                        } catch (Exception e) {
		                            // never crash - catch them all
        ...
        for (RouteInfo r :  routeDiff.added) {
			log("******updateRoutes: routeDiff.added - " + r.toString());
            if (isLinkDefault || ! r.isDefaultRoute()) {
                addRoute(newLp, r, TO_DEFAULT_TABLE, exempt);
				// deal with wlan0 ans usb0 co-exist issue
				log("******Adding route info: " + r.toString());
				LinkAddress destination = r.getDestination();
				log("******destination: " + destination.toString());
				InetAddress address_dest = InetAddress.parseNumericAddress("0.0.0.0");
				LinkAddress linkAddress_dest = new LinkAddress(address_dest, 0);
				log("******linkAddress_dest: " + linkAddress_dest.toString());
				if (destination.equals(linkAddress_dest)) {
					if ("usb0".equals(r.getInterface())) {
						mUsb0DefaultRouteInfo = r;
						// save the default usb0 route for quick add
						mUsb0DefaultRouteInfoQuickAdd = mUsb0DefaultRouteInfo;
						log("******Setting usb0 default route and save the default usb0 route for quick add: " + mUsb0DefaultRouteInfo.toString());
					}
					if ("wlan0".equals(r.getInterface())) {
						log("******Setting wlan0 default route: " + r.toString());
						if (mUsb0DefaultRouteInfo != null) {
							log("******usb0 had set a default gateway so remove it when wlan0 act as the default.");
							synchronized (mRoutesLock) {
		                        log("******Removing " + mUsb0DefaultRouteInfo + " for interface " + "usb0");
		                        try {
		                            mNetd.removeRoute("usb0", mUsb0DefaultRouteInfo);
									// set usb0 link info and route info to null so the connetivity will treat is as gone
									// but infact it exist in linx level.
									log("******Reseting the usb0 link info so the connectivity will treat is as gone");
									mUsb0DefaultRouteInfo = null;
								    mCurrentLinkProperties[TYPE_ETHERNET] = null;
		                        } catch (Exception e) {
		                            // never crash - catch them all
    ...
    private class NetworkStateTrackerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
				...
				case NetworkStateTracker.EVENT_TBOX_NETWORK_STATE_CHANGED: {
                    info = (NetworkInfo) msg.obj;
                    int type = info.getType();
                    if (type == mActiveDefaultNetwork) {
						log("******Broadcast - EVENT_TBOX_NETWORK_STATE_CHANGED: " + info + ", mActiveDefaultNetwork: " + mActiveDefaultNetwork);
						sendConnectedBroadcast(info);
                    } else {
						log("******NO Broadcast - EVENT_TBOX_NETWORK_STATE_CHANGED: " + info + ", mActiveDefaultNetwork: " + mActiveDefaultNetwork);
                    }
                    break;
                }
            }
