...
//[ADD-BEGIN] by kang
import android.os.SystemClock;
import android.location.GYRInfo; 
import android.location.A3DInfo;
import android.location.MOVInfo;
import android.location.IGYRListener;
import android.location.IA3DListener;
import android.location.IMOVListener;
//[ADD-END]
...
public class LocationManagerService extends ILocationManager.Stub {
    ...
    public static final boolean NAVI_DEBUG = SystemProperties.getBoolean("persist.sys.navi.debug", false);//[ADD] by kang
    ...
    //[ADD-BEGIN]by kang,24/10/2018
    private final Object mGYRLock = new Object();
    private final Object mA3DLock = new Object();
    private final Object mMOVLock = new Object();
    private final HashMap<Object, GYRReceiver> mGYRReceivers = new HashMap<Object, GYRReceiver>();
    private final HashMap<Object, A3DReceiver> mA3DReceivers = new HashMap<Object, A3DReceiver>();
    private final HashMap<Object, MOVReceiver> mMOVReceivers = new HashMap<Object, MOVReceiver>();
    //[ADD-END]by kang
    ...
    //[ADD-BEGIN] by kang
    @Override
    public void reportA3D(A3DInfo info){
        //if (NAVI_DEBUG) {
            //Log.d(TAG, "reportA3D elapsedRealtime = " + SystemClock.elapsedRealtime());
            //Log.d(TAG, "reportA3D: " + info.toString());
        //}

        handleA3DChanged(info);
    }

    @Override
    public void reportGYR(GYRInfo info){
        //if (NAVI_DEBUG) {
            //Log.d(TAG, "reportGYR elapsedRealtime = " + SystemClock.elapsedRealtime());
            //Log.d(TAG, "reportGYR: " + info.toString());
        //}

        handleGYRChanged(info);
    }

    @Override
    public void reportMOV(MOVInfo info){
        //if (NAVI_DEBUG) {
            //Log.d(TAG, "reportMOV elapsedRealtime = " + SystemClock.elapsedRealtime());
            //Log.d(TAG, "reportMOV: " + info.toString());
        //}

        handleMOVChanged(info);
    }
    //[[ADD-END]
    ...
    //[BEGIN-ADD] by kang,10/24/2018
    private final class A3DReceiver implements IBinder.DeathRecipient {
        final int mUid;  // uid of receiver
        final int mPid;  // pid of receiver
        final String mPackageName;  // package name of receiver
        final Object mKey;

        final IA3DListener mListenerA3D;


        public A3DReceiver(IA3DListener listener, int pid, int uid,
                String packageName, boolean hideFromAppOps){
            mPackageName = packageName;
            mListenerA3D = listener;
            mUid = uid;
            mPid = pid;
        ...
        public boolean isListener() {
            return mListenerA3D != null;
        }

        public IA3DListener getListener() {
            if (mListenerA3D != null) {
                return mListenerA3D;
            }
            throw new IllegalStateException("Request for non-existent listenerA3D");
        }

        @Override
        public void binderDied() {
            synchronized (mLock) {
                Log.e(TAG, "A3DListener died");
                removeA3DUpdates(mListenerA3D, mPackageName);
            }
        }
        ...
        public String toString() {
            StringBuilder s = new StringBuilder();
            s.append("A3DReceiver[");
            s.append(Integer.toHexString(System.identityHashCode(this)));
            if (mListenerA3D != null) {
                s.append(" listener :");
                s.append(mListenerA3D.toString());
            }
        ...
        public boolean callA3DChangedLocked(A3DInfo info) {
            if (mListenerA3D != null) {
                try {
                    synchronized (this) {
                        //if (NAVI_DEBUG) {
                            //Log.d(TAG, "callA3DChangedLocked elapsedRealtime = " + SystemClock.elapsedRealtime());
                        //}
                        
                        mListenerA3D.onA3DChanged(new A3DInfo(info));
                    }
                } catch (RemoteException e) {
                    return false;
                }
            } else {
                Slog.e(TAG, "ListenerA3D is null");
    ...
    private final class GYRReceiver implements IBinder.DeathRecipient{
        final int mUid;  // uid of receiver
        final int mPid;  // pid of receiver
        final String mPackageName;  // package name of receiver
        final Object mKey;

        final IGYRListener mListenerGYR;


        public GYRReceiver(IGYRListener listener, int pid, int uid,
                String packageName, boolean hideFromAppOps){
            mPackageName = packageName;
            mListenerGYR = listener;
            mUid = uid;
            mPid = pid;
            mKey = listener.asBinder();
        }
        ...
        public boolean isListener() {
            return mListenerGYR != null;
        }

        public IGYRListener getListener() {
            if (mListenerGYR != null) {
                return mListenerGYR;
            }
            throw new IllegalStateException("Request for non-existent listenerGYR");
        }

        @Override
        public void binderDied() {
            synchronized (mLock) {
                Log.e(TAG, "GYRListener died");
                removeGYRUpdates(mListenerGYR, mPackageName);
            }
        }
        ...
        public String toString() {
            StringBuilder s = new StringBuilder();
            s.append("GYRReceiver[");
            s.append(Integer.toHexString(System.identityHashCode(this)));
            if (mListenerGYR != null) {
                s.append(" listener :");
                s.append(mListenerGYR.toString());
            }
        ...
        public boolean callGYRChangedLocked(GYRInfo info) {
            if (mListenerGYR != null) {
                try {
                    synchronized (this) {
                        //if (NAVI_DEBUG) {
                            //Log.d(TAG, "callGYRChangedLocked elapsedRealtime = " + SystemClock.elapsedRealtime());
                        //}
                        
                        mListenerGYR.onGYRChanged(new GYRInfo(info));
                    }
                } catch (RemoteException e) {
                    return false;
                }
            } else {
                Slog.e(TAG, "ListenerGYR is null");
    ...
    @Override
    public void requestGYRUpdates(LocationRequest request, IGYRListener listener, String packageName){
        if(NAVI_DEBUG) Log.w(TAG, "requestGYRUpdates");
        ...
        long identity = Binder.clearCallingIdentity();
        try {
            synchronized (mGYRLock) {
                GYRReceiver recevier = checkGYRListenerOrIntentLocked(listener, pid, uid,
                        packageName, true);
            }
        } finally {
            Binder.restoreCallingIdentity(identity);
    ...
    @Override
    public void removeGYRUpdates(IGYRListener listener, String packageName){
        if(NAVI_DEBUG) Log.w(TAG, "removeGYRUpdates"); 
        ...
        synchronized (mGYRLock) {
            ...
            long identity = Binder.clearCallingIdentity();
            try {
                removeGYRUpdatesLocked(recevier);
            } finally {
                Binder.restoreCallingIdentity(identity);
            }
        }
    ...
    private void handleGYRChanged(GYRInfo info) {
        GYRInfo myInfo = new GYRInfo(info);
        
        synchronized (mGYRLock) {
            for(GYRReceiver v:mGYRReceivers.values()){
                if(null != v)
                v.callGYRChangedLocked(myInfo);
            }
        }
    }

    private GYRReceiver checkGYRListenerOrIntentLocked(IGYRListener listener,
             int pid, int uid, String packageName, boolean hideFromAppOps) {
         if (listener == null) {
             throw new IllegalArgumentException("need either listener");
         }else {
             return getGYRReceiverLocked(listener, pid, uid, packageName, hideFromAppOps);
         }
     }

    private GYRReceiver getGYRReceiverLocked(IGYRListener listener, int pid, int uid,
        String packageName, boolean hideFromAppOps) {
        IBinder binder = listener.asBinder();
        GYRReceiver receiver = mGYRReceivers.get(binder);
        if (receiver == null) {
            receiver = new GYRReceiver(listener, pid, uid, packageName,hideFromAppOps);
            mGYRReceivers.put(binder, receiver);
    ...
    private void removeGYRUpdatesLocked(GYRReceiver receiver){
        if (NAVI_DEBUG) Log.i(TAG, "remove " + Integer.toHexString(System.identityHashCode(receiver)));

        if (mGYRReceivers.remove(receiver.mKey) != null && receiver.isListener()) {
            receiver.getListener().asBinder().unlinkToDeath(receiver, 0);
        }
    }
    //[ADD-END] by kang

    //[ADD-BEGIN] by kang,01/11/2018
    @Override
    public void requestA3DUpdates(LocationRequest request, IA3DListener listener, String packageName){
        if(NAVI_DEBUG) Log.w(TAG, "requestA3DUpdates");
        ...
        long identity = Binder.clearCallingIdentity();
        try {
            synchronized (mA3DLock) {
                A3DReceiver recevier = checkA3DListenerOrIntentLocked(listener, pid, uid,
                        packageName, true);
            }
        } finally {
            Binder.restoreCallingIdentity(identity);
    ...
    @Override
    public void removeA3DUpdates(IA3DListener listener, String packageName) {
        if(NAVI_DEBUG) Log.w(TAG, "removeA3DUpdates");
        ...
        synchronized (mA3DLock) {
            ...
            long identity = Binder.clearCallingIdentity();
            try {
                removeA3DUpdatesLocked(recevier);
            } finally {
                Binder.restoreCallingIdentity(identity);
            }
        }
    ...
    private void handleA3DChanged(A3DInfo info) {
        A3DInfo myInfo = new A3DInfo(info);
        
        synchronized (mA3DLock) {
            for(A3DReceiver v:mA3DReceivers.values()){
                if(null != v)
                v.callA3DChangedLocked(myInfo);
            }
        }
    }
    
    private A3DReceiver checkA3DListenerOrIntentLocked(IA3DListener listener,
             int pid, int uid, String packageName, boolean hideFromAppOps) {
         if (listener == null) {
             throw new IllegalArgumentException("need either listener");
         } else {
             return getA3DReceiverLocked(listener, pid, uid, packageName, hideFromAppOps);
         }
     }

    private A3DReceiver getA3DReceiverLocked(IA3DListener listener, int pid, int uid,
        String packageName, boolean hideFromAppOps) {
        IBinder binder = listener.asBinder();
        A3DReceiver receiver = mA3DReceivers.get(binder);
        if (receiver == null) {
            receiver = new A3DReceiver(listener, pid, uid, packageName,hideFromAppOps);
            mA3DReceivers.put(binder, receiver);
    ...
    private void removeA3DUpdatesLocked(A3DReceiver receiver){
        if (NAVI_DEBUG) Log.i(TAG, "remove " + Integer.toHexString(System.identityHashCode(receiver)));

        if (mA3DReceivers.remove(receiver.mKey) != null && receiver.isListener()) {
            receiver.getListener().asBinder().unlinkToDeath(receiver, 0);
        }
    }
    //[END-ADD] by kang
    ...
    //[ADD-BEGIN] by kang
    private final class MOVReceiver implements IBinder.DeathRecipient{
        final int mUid;  // uid of receiver
        final int mPid;  // pid of receiver
        final String mPackageName;  // package name of receiver
        final Object mKey;

        final IMOVListener mListenerMOV;


        public MOVReceiver(IMOVListener listener, int pid, int uid,
                String packageName, boolean hideFromAppOps){
            mPackageName = packageName;
            mListenerMOV = listener;
            mUid = uid;
            mPid = pid;
            mKey = listener.asBinder();
        }
        ...
        public boolean isListener() {
            return mListenerMOV != null;
        }

        public IMOVListener getListener() {
            if (mListenerMOV != null) {
                return mListenerMOV;
            }
            throw new IllegalStateException("Request for non-existent listenerGYR");
        }
        ...
        @Override
        public void binderDied() {
            synchronized (mMOVLock) {
                Log.e(TAG, "MOVListener died");
                removeMOVUpdates(mListenerMOV, mPackageName);
            }
        }
        ...
        public String toString() {
            StringBuilder s = new StringBuilder();
            s.append("MOVReceiver[");
            s.append(Integer.toHexString(System.identityHashCode(this)));
            if (mListenerMOV != null) {
                s.append(" listener :");
                s.append(mListenerMOV.toString());
            }
        ...
        public boolean callMOVChangedLocked(MOVInfo info) {
            if (mListenerMOV != null) {
                try {
                    synchronized (this) {
                        //if (NAVI_DEBUG) {
                        ...
                        mListenerMOV.onMOVChanged(new MOVInfo(info));
                    }
                } catch (RemoteException e) {
                    return false;
                }
            } else {
                Slog.e(TAG, "ListenerMOV is null");
    ...
    @Override
    public void requestMOVUpdates(LocationRequest request, IMOVListener listener, String packageName){
        if(NAVI_DEBUG) Log.w(TAG, "requestMOVUpdates");
        ...
        try {
            synchronized (mMOVLock) {
                MOVReceiver recevier = checkMOVListenerOrIntentLocked(listener, pid, uid,
                        packageName, true);
            }
        } finally {
            Binder.restoreCallingIdentity(identity);
    ...
    @Override
    public void removeMOVUpdates(IMOVListener listener, String packageName) {
        if(NAVI_DEBUG) Log.w(TAG, "removeMOVUpdates");
        ...
        synchronized (mMOVLock) {
            ...
            long identity = Binder.clearCallingIdentity();
            try {
                removeMOVUpdatesLocked(recevier);
            } finally {
                Binder.restoreCallingIdentity(identity);
            }
        }
    ...
    private void handleMOVChanged(MOVInfo info) {
        MOVInfo myInfo = new MOVInfo(info);
        
        synchronized (mMOVLock) {
            for(MOVReceiver v:mMOVReceivers.values()){
                if(null != v)
                v.callMOVChangedLocked(myInfo);
            }
        }
    }
    
    private MOVReceiver checkMOVListenerOrIntentLocked(IMOVListener listener,
             int pid, int uid, String packageName, boolean hideFromAppOps) {
         if (listener == null) {
             throw new IllegalArgumentException("need either listener");
         } else {
             return getMOVReceiverLocked(listener, pid, uid, packageName, hideFromAppOps);
         }
     }

    private MOVReceiver getMOVReceiverLocked(IMOVListener listener, int pid, int uid,
        String packageName, boolean hideFromAppOps) {
        IBinder binder = listener.asBinder();
        MOVReceiver receiver = mMOVReceivers.get(binder);
        if (receiver == null) {
            receiver = new MOVReceiver(listener, pid, uid, packageName,hideFromAppOps);
            mMOVReceivers.put(binder, receiver);
    ...
    private void removeMOVUpdatesLocked(MOVReceiver receiver){
        if (NAVI_DEBUG) Log.i(TAG, "remove " + Integer.toHexString(System.identityHashCode(receiver)));

        if (mMOVReceivers.remove(receiver.mKey) != null && receiver.isListener()) {
            receiver.getListener().asBinder().unlinkToDeath(receiver, 0);
        }
    }
    //[END-ADD] by kang
