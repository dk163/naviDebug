...
//gaorenwei  20170801 add for get  RTC time 
static jlong android_os_SystemClock_getCurrentTimeMills(JNIEnv* env,
        jobject clazz)
{
			int fd;
			int res;
	...
	ts.tv_sec = 0;
    ts.tv_nsec = 0;
    res = ioctl(fd, ANDROID_ALARM_GET_TIME(0), &ts);
    if(res < 0) {
        ALOGW("Unable to get rtc  %s\n",strerror(errno));
        ret = -1;
    }
	ret = ts.tv_sec;
    close(fd);
...
static JNINativeMethod gMethods[] = {
    ...
    {"getCurrentTimeMillis",      "()J", 
    		(void*) android_os_SystemClock_getCurrentTimeMills },
};
