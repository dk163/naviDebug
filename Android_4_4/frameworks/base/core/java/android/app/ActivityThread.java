...
public final class ActivityThread {
    ...
    private void handleCreateService(CreateServiceData data) {
        ...
        try {
            ...
            if (!mInstrumentation.onException(service, e)) {
				// TODO: Just workaround for FOTA service as it load a 0x00 res ID after first update OS.
				if (!"com.incall.apps.fota.services.UpdateService".equals(data.info.name)) {
		            throw new RuntimeException(
		                "Unable to create service " + data.info.name
		                + ": " + e.toString(), e);
				} else {
					Slog.v(TAG, "Workaround for exception: Unable to create service " + data.info.name
                    	+ ": " + e.toString(), e);
				}
            }
        }
