 ...
 *
 ******************************************************************************
 * $Id: AssistNowLeg.cpp 114239 2016-05-02 13:54:03Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/v3.30/gps/agnss/AssistNowLeg.cpp $
 *****************************************************************************/
                                             ...
                                             CONF_t *cb)
{
    ...
    if(!_singleton)
    {
        _singleton = new (std::nothrow) CAssistNowLeg(online_primary_server, online_secondary_server, offline_primary_server, offline_secondary_server, server_token, cb);
        tmpRef=_singleton;
    }
    pthread_mutex_unlock(&_singleton_mutex);
...
*/
void CAssistNowLeg::sWriteToRcv( const void *context
                                  ...
                                  , UBX_I4 evtInfoSize )
{
    if(pContext)
    {
        ((CAssistNowLeg *) const_cast<void *>(pContext))->evtProgHandler( evtType
                                                                        , pEvtInfo
                                                                        , evtInfoSize );
    }
}
                                  ...
                                  , UBX_I4 evtInfoSize )
{
    ...
    switch(evtType)
    {
        ...
        case MGA_PROGRESS_EVT_MSG_SENT:
        {
            ...
            {
                unsigned int sv = pMsgInfo->pMsg[6]
                                | pMsgInfo->pMsg[7] << 8
                                | pMsgInfo->pMsg[8] << 16
                                | pMsgInfo->pMsg[9] << 24;
                snprintf( msgTxt + strlen(msgTxt)
                        , TMP_STR_LEN - strlen(msgTxt), " for SV %i"
                        , sv );
            }
        ...
        case MGA_PROGRESS_EVT_LEGACY_AIDING_SERVER_UPDATE_RECEIVED:
        {
            ...
            print_std("Received request from receiver to update Alp fileId %d offset %d size %d", cli.fileId, offset_in_bytes, size_in_bytes);
            break;
        }
        ...
        {
            print_err("Could not send the expected request to the receiver due to lack of memory!", msgTxt);
            _evt_handler_status=FAIL;
            break;
        }
...
bool CAssistNowLeg::initLegTransfer( bool online )
{
        ...
        else
        {
            ...
            switch(res)
            {
                case MGA_API_NO_DATA_TO_SEND:
                    print_err("Data from Server does not contain any LEG %s data.", agnssServiceTypeToString(service));
                    break;
                ...
                case MGA_API_NO_MGA_INI_TIME:
                    print_err("No UBX-AID-INI message in data from LEG %s service.",agnssServiceTypeToString(service));
                    break;

                case MGA_API_OUT_OF_MEMORY:
                    print_err("Run out of memeory while processing data from LEG %s service.",agnssServiceTypeToString(service));
                    break;

                default:
                    print_err("Unknown error occured while processing data from LEG %s service: %d",agnssServiceTypeToString(service), res);
                    break;
            }
        }
    }
    ...
    {
        print_err("Getting Assistance data from lEG %s service failed",agnssServiceTypeToString(service));
    }
    return result;
}
...
/*! Helper function which acts as a wrapper for some libMga functions
    ...
    \return             SUCCESS if the transfer completed successfully,
                        ...
                        to complete
*/
TRI_STATE_FUNC_RESULT_t CAssistNowLeg::continueLegTransfer( unsigned char const *buf
                                                          , size_t size)
{
    MGA_API_RESULT res = MGA_API_OK;
    TRI_STATE_FUNC_RESULT_t result=FAIL;
