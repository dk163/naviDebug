...
#if defined(__linux__) || defined(__FreeBSD__) || defined(ANDROID_BUILD)  /* Linux & FreeBSD */

#define RS232_PORTNR   1
...
char *comports[RS232_PORTNR]={"/dev/ttymxc0"};//[ADD] by kang
...
int RS232_GetPortnr(const char *devname)
{
  ...
  {
	  UBX_LOG(LCAT_INFO, "port: %s", comports[i]);
...
int RS232_GetPortFD(int comport_number) {
