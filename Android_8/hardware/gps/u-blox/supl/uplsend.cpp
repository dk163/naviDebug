 ...
 *
 ******************************************************************************
 * $Id: uplsend.cpp 108056 2015-12-11 14:22:41Z marcus.picasso $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/v3.30/gps/supl/uplsend.cpp $
 *****************************************************************************/
	...
	else
	{
		...
		else
		{
			...
			if (ipAddr.s_addr != 0)
			{
				...
				UBX_LOG(LCAT_VERBOSE, "Type AGPS_SETID_TYPE_NONE - IP Address %u.%u.%u.%u",
...
static void fillLocation(LocationId_t *pLocId)
{
	...
	UBX_LOG(LCAT_VERBOSE, "Location type %d (%s)", refloc.type, _LOOKUPSTR(refloc.type, AGpsRefLocation));
	...
	case AGPS_REF_LOCATION_TYPE_GSM_CELLID:
        ...
        UBX_LOG(LCAT_VERBOSE, "GSM (2G) cell, cell mcc: %ld, mnc: %ld, lac: %ld uc: %ld",  pLocId->cellInfo.choice.gsmCell.refMCC, pLocId->cellInfo.choice.gsmCell.refMNC, pLocId->cellInfo.choice.gsmCell.refLAC, pLocId->cellInfo.choice.gsmCell.refCI);
        break;
		...
		pLocId->status = Status_current;
		UBX_LOG(LCAT_VERBOSE, "WCDMA (3G) cell mcc: %ld, mnc: %ld, uc: %ld", pLocId->cellInfo.choice.wcdmaCell.refMCC, pLocId->cellInfo.choice.wcdmaCell.refMNC, pLocId->cellInfo.choice.wcdmaCell.refUC);
		break;
...
static void sendAndFree(SUPL_CONNECTION *supl_connection, ULP_PDU_t *pMsg)
{
    ...
    {
        UBX_LOG(LCAT_ERROR, "Strange, dimension do not fit.. (buf[0] = 0x%.2X, buf[1] = 0x%.2X)", buf[0],buf[1]);
    }
...
static struct Position *allocatePosition(double lat, double lon)
{
    ...
    UBX_LOG(LCAT_VERBOSE, "Seconds=%d,Minutes=%d,Hour=%d", curTime.tm_sec,curTime.tm_min,curTime.tm_hour);
