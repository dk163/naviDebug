 ...
 *
 ******************************************************************************
 * $Id: database.h 108174 2015-12-14 13:11:31Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/PRODUCTS/libParser/tags/libParser_v1.04/database.h $
 *****************************************************************************/
/*!
  \file
...
public:
    typedef void (*PARSE_FUNC_t)(unsigned char* pBuffer, int iSize, CDatabase* pDatabase);
...
#define DATA_SATELLITES_IN_VIEW_GNSSID_(ix)		(CDatabase::DATA_t)(CDatabase::DATA_SATELLITES_IN_VIEW_GNSSID + ix)
        DATA_SATELLITES_IN_VIEW_GNSSID_X		= MAX_SATELLITES_IN_VIEW + DATA_SATELLITES_IN_VIEW_GNSSID,
        DATA_SATELLITES_IN_VIEW_SVID,
#define DATA_SATELLITES_IN_VIEW_SVID_(ix)		(CDatabase::DATA_t)(CDatabase::DATA_SATELLITES_IN_VIEW_SVID + ix)
...
#define DATA_UBX_SATELLITES_IN_VIEW_NAV_STA_(ix)	(CDatabase::DATA_t)(CDatabase::DATA_UBX_SATELLITES_IN_VIEW_NAV_STA + ix)
        DATA_UBX_SATELLITES_IN_VIEW_NAV_STA_X		= MAX_SATELLITES_IN_VIEW + DATA_UBX_SATELLITES_IN_VIEW_NAV_STA,
...
#define DATA_UBX_SATELLITES_IN_MEAS_DOPPLER_(ix)		(CDatabase::DATA_t)(CDatabase::DATA_UBX_SATELLITES_IN_MEAS_DOPPLER + ix)
        ...
        ///////////////////////////////////////////////////////////////////////////////////
        // the following fields can not be set by the parser, they have to be calculated //
        ///////////////////////////////////////////////////////////////////////////////////
...
public:
    ...
    virtual int Process(MSG_t type, unsigned char * pBuffer, int iSize, PARSE_FUNC_t func);
...
*/
    ...
    template<typename T> bool GetOutput(DATA_t data, T& v)
    {
        return m_varO[data].Get(v);
    }

    bool GetOutput(DATA_t data, TIMESTAMP& rTS) const
    {
        return m_varO[data].Get(rTS);
    }

    CVar* GetOutputArrayBase(DATA_t data)
    {
        return &m_varO[data];
    }
...
protected:
    ...
    MSG_t m_epochBegin;				// Defines the first message of an epoch that has to arrive to allow writing to the database
    MSG_t m_epochEnd;				// Defines the last message of an epoch that has to arrive to allow committing the database
    STATE_t m_vasS;					// Status of GNSS reception
    bool m_receivingEpoch;			// Indicates if we are in the process of receiving an epoch (database is writable)

    char* m_pNmeaMsgListNext;
    char* m_pNmeaMsgListOutput;
