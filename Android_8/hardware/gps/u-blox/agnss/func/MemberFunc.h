 ...
 *
 ******************************************************************************
 * $Id: MemberFunc.h 103715 2015-10-06 11:18:18Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/v3.30/gps/agnss/func/MemberFunc.h $
 *****************************************************************************/
...
#define FREF(a, b, c) (CMemberFunc<a>(b, &a::c)) //!< This acts as a wrapper for the constructor of \ref CMemberFunc
...
*/
template <class T> CMemberFunc<T>::CMemberFunc(T* obj, DATA_FUNC_p func)
    : _obj(obj)
    , _func(func)
{
}
...
/*! Pass data on to the function pointer
    ...
    \return                  The return value returned by the function pointer.
                             ...
                             properly
*/
template <class T> TRI_STATE_FUNC_RESULT_t CMemberFunc<T>::passData(unsigned char const * data, size_t size)
{
    if(!_obj || !_func)
        return FAIL;
