 ...
 *
 ******************************************************************************
 * $Id: AssistNowMga.cpp 113551 2016-04-01 12:00:45Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/v3.30/gps/agnss/AssistNowMga.cpp $
 *****************************************************************************/
...
pthread_mutex_t CAssistNowMga::_singleton_mutex=PTHREAD_MUTEX_INITIALIZER;
...
CAssistNowMga::CAssistNowMga(const char *online_primary_server,
    ...
    , _ubx_poll_mga_dbd((void*)this, CAgnss::writeToRcv, 30)
    , _action_data_id(0)
    , _num_get_gnss_send_tries(-1)
    , _num_get_gnss_max_send_tries(5)
    , _num_get_gnss_recv_tries(0)
    , _num_get_gnss_max_recv_tries(15)
    , _num_config_ack_send_tries(0)
    , _num_config_ack_max_send_tries(5)
    , _transfer_buf(NULL)
    , _transfer_size(0)
    , _todays_data(NULL)
    , _todays_data_size(0)
    , _num_msgs_expected(0)
    ...
    , _flow_config(NULL)
{
    ...
    _online_config.strPrimaryServer   = online_primary_server ?
                                         strdup(online_primary_server)
                                         : strdup("");
    _online_config.strSecondaryServer = online_secondary_server ?
                                         strdup(online_secondary_server)
                                         : strdup("");
    _online_config.strServerToken     = server_token ?
                                         strdup(server_token)
                                         : strdup("");
    _online_config.dataTypeFlags = MGA_DATA_EPH
                                 | MGA_DATA_ALM
                                 | MGA_DATA_AUX;
    ...
    _offline_config.strPrimaryServer   = offline_primary_server ?
                                          strdup(offline_primary_server)
                                          : strdup("");
    _offline_config.strSecondaryServer = offline_secondary_server ?
                                          strdup(offline_secondary_server)
                                          : strdup("");
    _offline_config.strServerToken     = server_token ?
                                          strdup(server_token)
                                          : strdup("");
    ...
    _fun_lookup[TIME]            [TRANSFER ].push(FREF(CAssistNowMga, this, configureAidAck));
    _fun_lookup[TIME]            [TRANSFER ].push(FREF(CAssistNowMga, this, timeTransfer));
    _fun_lookup[TIME]            [TRANSFER ].push(FREF(CAssistNowMga, this, finishTimeTransfer));

    _fun_lookup[POSITION]        [TRANSFER ].push(FREF(CAssistNowMga, this, configureAidAck));
    _fun_lookup[POSITION]        [TRANSFER ].push(FREF(CAssistNowMga, this, timeTransfer));
    _fun_lookup[POSITION]        [TRANSFER ].push(FREF(CAssistNowMga, this, posTransfer));

    _fun_lookup[ONLINE]          [DOWNLOAD ].push(FREF(CAssistNowMga, this, getEnabledGnssFromRcv));
    _fun_lookup[ONLINE]          [DOWNLOAD ].push(FREF(CAssistNowMga, this, onlineDownload));

    _fun_lookup[ONLINE]          [TRANSFER ].push(FREF(CAssistNowMga, this, onlineTransfer));

    _fun_lookup[OFFLINE]         [DOWNLOAD ].push(FREF(CAssistNowMga, this, getEnabledGnssFromRcv));
    _fun_lookup[OFFLINE]         [DOWNLOAD ].push(FREF(CAssistNowMga, this, offlineDownload));

    _fun_lookup[OFFLINE]         [TRANSFER ].push(FREF(CAssistNowMga, this, offlineTransfer));

    _fun_lookup[RECV_AID_STATE]  [POLL_RECV].push(FREF(CAssistNowMga, this, recvStatePoll));

    _fun_lookup[RECV_AID_STATE]  [TRANSFER ].push(FREF(CAssistNowMga, this, configureAidAck));
    _fun_lookup[RECV_AID_STATE]  [TRANSFER ].push(FREF(CAssistNowMga, this, timeTransfer));
    _fun_lookup[RECV_AID_STATE]  [TRANSFER ].push(FREF(CAssistNowMga, this, recvStateTransfer));
}
                                             ...
                                             CONF_t *cb)
{
    ...
    if(!_singleton)
    {
        _singleton = new (std::nothrow) CAssistNowMga(online_primary_server, online_secondary_server, offline_primary_server, offline_secondary_server, server_token, cb);
        tmpRef=_singleton;
    }
    pthread_mutex_unlock(&_singleton_mutex);
    return tmpRef;
...
CList<CFuncMngr> CAssistNowMga::impl_initAction( SERVICE_t service
                                                , ACTION_t action)
{
...
bool CAssistNowMga::impl_isValidServiceAction( SERVICE_t service
                                             , ACTION_t action)
{
    bool result=false;
...
bool CAssistNowMga::impl_isValidData(SERVICE_t service, unsigned char const *buf, size_t size)
{
    ...
    switch(service)
    {
        case RECV_AID_STATE:
        {
            result=(verifyUbxMsgsBlock(buf, size, _recv_state_allowed, sizeof(_recv_state_allowed), NULL)>0);
            break;
        }
        case ONLINE:
        {
            result=(verifyUbxMsgsBlock(buf, size, _online_allowed, sizeof(_online_allowed), NULL)>0);
            break;
        }
        case OFFLINE:
        {
            result=(verifyUbxMsgsBlock(buf, size, _offline_allowed, sizeof(_offline_allowed), NULL)>0);
            break;
        }
        case TIME:
        case _NUM_SERVICES_:
...
uint16_t CAssistNowMga::impl_getDataId()
{
...
/*! This function will first check which GNSSs the receiver supports and
    ...
    \return             SUCCESS if the download completed successfully,
                        ...
                        to complete
*/
TRI_STATE_FUNC_RESULT_t CAssistNowMga::offlineDownload( unsigned char const *buf
                                                      , size_t size )
{
    (void) (buf);
    (void) (size);
    ...
    int tmpSize=0;
    if( initMga()
     && mgaGetOfflineData(&_offline_config, &tmpBuf, &tmpSize)==MGA_API_OK
     && tmpBuf
     && tmpSize>0)
    {
...
/*! This function will first check which GNSSs the receiver supports and
    ...
    \return             SUCCESS if the download completed successfully,
                        ...
                        to complete
*/
TRI_STATE_FUNC_RESULT_t CAssistNowMga::onlineDownload( unsigned char const *buf
                                                     , size_t size)
{
    (void) (buf);
    (void) (size);
    ...
    int tmpSize=0;
    if( initMga()
     && mgaGetOnlineData(&_online_config, &tmpBuf, &tmpSize)==MGA_API_OK
     && tmpBuf
     && tmpSize>0)
    {
...
/*! This function will try to transfer the offline data
    ...
    \return             SUCCESS if the transfer completed successfully,
                        ...
                        to complete
*/
TRI_STATE_FUNC_RESULT_t CAssistNowMga::offlineTransfer( unsigned char const *buf
                                                      , size_t size )
{
    TRI_STATE_FUNC_RESULT_t result=FAIL;
    if(!_transfer_buf)
...
/*! This function will try to transfer the online data
    ...
    \return             SUCCESS if the transfer completed successfully,
                        ...
                        to complete
*/
TRI_STATE_FUNC_RESULT_t CAssistNowMga::onlineTransfer( unsigned char const *buf
                                                     , size_t size )
{
    TRI_STATE_FUNC_RESULT_t result=FAIL;
    if(!_transfer_buf)
...
bool CAssistNowMga::initMgaTransfer( bool online )
{
    ...
    if (initMga())
    {
        ...
        else
        {
            ...
            _todays_data=NULL;
            _todays_data_size=0;
            res = mgaGetTodaysOfflineData(&nowTm, _transfer_buf, _transfer_size, &_todays_data, &_todays_data_size);
            if(res==MGA_API_OK)
            {
                res = mgaSessionSendOfflineData(_todays_data, _todays_data_size, &curr_time);
                if(res!=MGA_API_OK)
                {
                    print_err("An error occured while trying to send AssistNow Offline data to the receiver. Abort");
                }
            }
            else
            {
                print_err("An error occured while trying to select the current AssistNow Offline data. Abort");
            }
        }
        ...
        else
        {
            ...
            switch(res)
            {
                case MGA_API_NO_DATA_TO_SEND:
                    print_err("Data from Server does not contain any MGA %s data.", agnssServiceTypeToString(service));
                    break;
                ...
                case MGA_API_NO_MGA_INI_TIME:
                    print_err("No MGA-INI-TIME message in data from MGA %s service.",agnssServiceTypeToString(service));
                    break;

                case MGA_API_OUT_OF_MEMORY:
                    print_err("Run out of memeory while processing data from MGA %s service.",agnssServiceTypeToString(service));
                    break;

                default:
                    print_err("Unknown error occured while processing data from MGA %s service: %d",agnssServiceTypeToString(service), res);
                    break;
            }
        }
    }
    ...
    {
        print_err("Getting Assistance data from MGA %s service failed",agnssServiceTypeToString(service));
    }
    return result;
}
...
/*! Helper function which acts as a wrapper for some libMga functions
    ...
    \return             SUCCESS if the transfer completed successfully,
                        ...
                        to complete
*/
TRI_STATE_FUNC_RESULT_t CAssistNowMga::continueMgaTransfer( unsigned char const *buf
                                                          , size_t size)
{
    MGA_API_RESULT res = MGA_API_OK;
    TRI_STATE_FUNC_RESULT_t result=FAIL;
...
/*! Function which will perform the transfer of the previously polled
    ...
    \return             SUCCESS if the transfer completed successfully,
                        ...
                        to complete
*/
TRI_STATE_FUNC_RESULT_t CAssistNowMga::recvStateTransfer( unsigned char const *buf
                                                        , size_t size )
{
    TRI_STATE_FUNC_RESULT_t result=FAIL;
    if(!_ubx_mga_dbd.hasSentData())
    {
        ...
        if(tmpSize>=0)
        {
            if( _ubx_mga_dbd.setData(tmpBuf, tmpSize)
             && _ubx_mga_dbd.sendData() )
            {
                result=CALL_AGAIN;
            }
            free(tmpBuf);
...
*/
TRI_STATE_FUNC_RESULT_t CAssistNowMga::recvStatePoll( unsigned char const *buf
                                                    , size_t size )
{
    TRI_STATE_FUNC_RESULT_t result=FAIL;
...
/*! This function will try to transferring the current time to the receiver.
    ...
    \return             SUCCESS if the transfer completed successfully,
                        ...
                        to complete
*/
TRI_STATE_FUNC_RESULT_t CAssistNowMga::timeTransfer( unsigned char const *buf
                                                   , size_t size)
{
    TRI_STATE_FUNC_RESULT_t result=FAIL;
    ...
    if(!_ubx_mga_ini_time.isReadyToSend())
    {
         ...
         && createUbxAidIni(&Payload, &accTimeNow))
        {
            ...
            if( _ubx_mga_ini_time.setTime(&accTimeNow)
             && _ubx_mga_ini_time.sendData())
            {
...
/*! This function will try to transfer the current position to the receiver.
    ...
    \return             SUCCESS if the transfer completed successfully,
                        ...
                        to complete
*/
TRI_STATE_FUNC_RESULT_t CAssistNowMga::posTransfer( unsigned char const *buf
                                                  , size_t size)
{
    TRI_STATE_FUNC_RESULT_t result=FAIL;
    ...
    if(!_ubx_mga_ini_pos.isReadyToSend())
    {
        ...
        if( getCurrentPosition(&pos)
         && _ubx_mga_ini_pos.setPos(&pos)
         && _ubx_mga_ini_pos.sendData())
        {
...
/*! This function will try set the action id to a valid  transferring the current time to the receiver.
    ...
    \return             SUCCESS if the transfer completed successfully,
                        ...
                        to complete
*/
TRI_STATE_FUNC_RESULT_t CAssistNowMga::finishTimeTransfer( unsigned char const *buf
                                                         , size_t size )
{
    ((void)(buf));
    ((void)(size));
...
/*! This helper function will send a UBX message to request the enabled
    ...
    \param buf        : A message that will be parsed for supported GNSS (May be NULL)
    \param size       : The size of buf (must be 0 if buf is NULL)
    \return             SUCCESS if the information was retrieved from the receiver
                        ...
                        to complete
*/
TRI_STATE_FUNC_RESULT_t CAssistNowMga::getEnabledGnssFromRcv( unsigned char const *buf
                                                            , size_t size)
{
    int supported=0;
    ...
    if(_num_get_gnss_send_tries < 0 || _num_get_gnss_recv_tries >= _num_get_gnss_max_recv_tries)
    {
        ...
        {
            print_std("Tried to send the request for the enabled GNSS from the receiver %i times. Receiver not responding", _num_get_gnss_max_send_tries);
            _num_get_gnss_send_tries=-1;
            _num_get_gnss_recv_tries = 0;
            result=FAIL;
        }
    ...
    else
    {
        ...
        if(buf && size)
        {
                    ...
                    && (buf[2] == 0x06) && (buf[3] == 0x3E)) // CFG-GNSS
            {
                ...
                {
                    print_err("Unknown version of the UBX message containing the list of supported receivers: %u", buf[6]);
                }
            }
        }
    }
...
*/
void CAssistNowMga::sWriteToRcv( const void *context
                                  ...
                                  , UBX_I4 evtInfoSize )
{
    if(pContext)
    {
        ((CAssistNowMga *) const_cast<void *>(pContext))->evtProgHandler( evtType
                                                                        , pEvtInfo
                                                                        , evtInfoSize );
    }
}
...
/*! This function will configure the receiver to send out acknowledgments
    ...
    \return             SUCCESS if the configuration completed successfully,
                        ...
                        to complete
*/
TRI_STATE_FUNC_RESULT_t CAssistNowMga::configureAidAck( unsigned char const *buf
                                                      , size_t size)
{
    TRI_STATE_FUNC_RESULT_t result=FAIL;
    ...
    if(_ubx_cfg_navx5.hasSentData())
    {
        ...
        if(result==SUCCESS)
        {
            print_std("Acknowledgment for aiding messages enabled (Try %u/%u)",  _num_config_ack_send_tries, _num_config_ack_max_send_tries);
            _num_config_ack_send_tries=0;
        ...
        else if(result==FAIL)
        {
            ...
            {
                print_err("Did not receive acknowledgement from receiver that enabling acknowledgment for aiding messages worked in %u tries. Abort.", _num_config_ack_max_send_tries);
                sendDataIfRequired=false;
                _num_config_ack_send_tries=0;
            }
            else
            {
                print_std("Received no acknowledgment for enabling acknowledgments for aiding in time (Try %u/%u)",  _num_config_ack_send_tries, _num_config_ack_max_send_tries);
            }
        }
    }
    ...
    {
        print_std("Configure receiver to acknowledge aiding (Try %u/%u)", _num_config_ack_send_tries+1, _num_config_ack_max_send_tries);
                                  ...
                                  , UBX_I4 evtInfoSize )
{
    ...
    switch(evtType)
    {
        ...
        case MGA_PROGRESS_EVT_MSG_SENT:
        {
            ...
            {
                snprintf(msgTxt + strlen(msgTxt), TMP_STR_LEN - strlen(msgTxt), "%s",
                         gnssIdToString(gnssId));
            }
            ...
            if (needSvData) //Add SV Data to the output
            {
                ...
                snprintf(msgTxt + strlen(msgTxt), TMP_STR_LEN - strlen(msgTxt), " for SV %i",
                         sv);
            }
        ...
        case MGA_PROGRESS_EVT_TERMINATED:
        {
            ...
            if(*stopReason!=TERMINATE_HOST_CANCEL)
            {
                switch(*stopReason)
                {
                    ...
                    {
                        snprintf(msgTxt + strlen(msgTxt), TMP_STR_LEN - strlen(msgTxt),
                                 "NAK from receiver. Data received, but receiver had a problem processing it. Consider all data invalidated.");
                        break;
                    }
        ...
        {
            MgaMsgInfo const *pMsgInfo = (MgaMsgInfo const *) pEvtInfo;
            snprintf(msgTxt, TMP_STR_LEN, "Message [%i], transfer failed. Reason [%i] - ",
                     pMsgInfo->sequenceNumber + 1, pMsgInfo->mgaFailedReason);
            ...
            switch(pMsgInfo->mgaFailedReason)
            {
                ...
                case MGA_FAILED_COULD_NOT_STORE:
                    ...
                    snprintf(msgTxt + strlen(msgTxt), TMP_STR_LEN - strlen(msgTxt),
                             "Message could not be stored. The receiver does not require this information");
                    break;
        ...
        {
            MgaMsgInfo const *pMsgInfo = (MgaMsgInfo const *) pEvtInfo;
            print_std("Transfer of Message [%i] completed successfully.",
                   pMsgInfo->sequenceNumber + 1);
            break;
        }
        ...
        case MGA_PROGRESS_EVT_SERVICE_ERROR:
        {
            ...
            switch(pMsgInfo->errorType)
            {
                ...
                {
                    snprintf(msgTxt + strlen(msgTxt), TMP_STR_LEN - strlen(msgTxt),
                             "Bad status: '%s'", pMsgInfo->errorMessage);
                    break;
                }
