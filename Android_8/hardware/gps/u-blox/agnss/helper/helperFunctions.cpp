 ...
 *
 ******************************************************************************
 * $Id: helperFunctions.cpp 111625 2016-02-26 07:40:03Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/v3.30/gps/agnss/helper/helperFunctions.cpp $
 *****************************************************************************/
...
static char const *const s_legDataIdToTxt[] =
{
    "EPH",
    "ALM",
    "HUI",
...
static char const *const s_mgaDataIdToTxt[] =
{
    "EPH",
    "ALM",
    "TIMEOFFSET",
...
static char const *const s_mgaIniIdToTxt[] =
{
    "MGA-INI-POS_XYZ",
    "MGA-INI-POS_LLH",
    "MGA-INI-TIME_UTC",
...
static char const *const s_gnssIdToTxt[] =
{
    "GPS",
    "SBAS",
    "GAL",
...
static char const *const s_agnssServiceIdToTxt[] =
{
    "Time",
    "Position",
    "Receiver Aiding State",
    "Offline",
    "Online"
};

//! Translates AGNSS Action IDs to text
static char const *const s_agnssActionIdToTxt[] =
{
    "Transfer",
    "Download",
    "Poll"
...
*/
extern char const *_strLookup(unsigned int const v, char const *const *l, unsigned int n);
#ifndef _LOOKUPSTR
# define _LOOKUPSTR(v, t) _strLookup(v, t, sizeof(t)/sizeof(t[0]) )
#endif

char const * gnssIdToString(unsigned int gnssId)
{
    return _LOOKUPSTR(gnssId, s_gnssIdToTxt);
}
...
char const * ubxIdsToString(unsigned int classId, unsigned int msgId)
{
    ...
    return _LOOKUPSTR(id, s_legDataIdToTxt);
}
                          ...
                          , BUFC_t **msgs )
{
    ...
    if(!tmpBuf)
        return HELPER_ERR_OUT_OF_MEMORY;
    ...
    do
    {
        ssize_t msg_length=isUbxMessage(tmpBuf, tmpSize, true);
        if(msg_length < 0)
        {
        ...
        else // Valid ubx message found
        {
            ...
            if(allowed && allowed_num && !isAllowedUbxMsg(tmpBuf, msg_length, allowed, allowed_num))
            {
            ...
            else // Msg is allowed or no list of allowed messages provided
            {
                ...
                if(result>=0)
                {
                    ...
                    memmove(tmpBuf, &tmpBuf[msg_length], tmpSize);
    ...
    // Always free the temporary buffer. This was only used for
...
bool isAllowedUbxMsg(unsigned char const * buf, int size, UBX_MSG_TYPE const * allowed, size_t allowed_num, bool enWildCards /* = false */)
{
    ...
    for(size_t i=0; i < allowed_num && !allowedMsg ; ++i)
    {
        ...
        if( buf[2] != 0xFF && buf[3] != 0xFF  // No messages with class or message Id 0xFF exist, we use them as wildcards
        ...
        {
            if( ( allowed[i].clsId == 0xFF && allowed[i].msgId == 0xFF )      // All messages of all classes allowed
             || ( buf[2] == allowed[i].clsId && allowed[i].msgId == 0xFF ) )  // All messages of a class allowed
            {
                allowedMsg=true;
            }
        }
...
int verifyAlpData(const unsigned char* msg, unsigned int size, ALP_FILE_HEADER_t *header)
{
    ...
    memcpy(&tmp, msg, sizeof(ALP_FILE_HEADER_t));
    ...
    if(header)
        memcpy(header, &tmp, sizeof(ALP_FILE_HEADER_t));
    ...
    while( !con_ended )
    {
        ...
        if( poll(&pfd, 1, timeoutMs) > 0 )
        {
            ...
            if( pfd.revents & POLLIN
             && ioctl(fd, FIONREAD, &bytes_available)==0
             && bytes_available>0)
            {
                ...
                do
                {
                    ...
                    else if( ret == 1 )
                    {
                        ...
                        if(stopSeq && iSize >= stopSeqSize)
                        {
                            ...
                            if( memcmp( &(buf[iSize-stopSeqSize])
                              , stopSeq
                              , stopSeqSize) == 0 )
                            {
                    ...
                    if( !con_ended
                     && (ssize_t) bufSize - (ssize_t) iSize <= 0 )
                    {
...
ssize_t encodeAgnssf00(unsigned char **raw, BUF_t const * bufs, int8_t numBufs)
{
    ...
    ssize_t result=HELPER_ERR_UNKNOWN_ERROR;
...
int8_t decodeAgnssf00(unsigned char const *raw, size_t rawSize, BUF_t * bufs, int8_t numBufs)
{
    ...
    memset(tmpBufs, 0, 0x7F*sizeof(unsigned char *));
...
ssize_t createUbx( unsigned char **buf
                 ...
                 , const void* pData2, size_t iData2
                 , const void* pData3, size_t iData3
                 , const void* pData4, size_t iData4)
{
                 ...
                 , CList<BUFC_t> payload )
{
    ...
    for(CList<BUFC_t>::CIter i(payload); i.isValid(); ++i)
    {
        if(sizeBuffer >= 0)
        {
            if((i->p && i->i) || (!i->p && !i->i))
...
void relToAbsGpsTime(GPS_TIME_t *gpsTime)
{
    if(gpsTime)
    {
        ...
        assert(gpsTimeRes); ((void)(gpsTimeRes));
                  ...
                  , bool unixNeedsLeap /* = false */ )
{
    ...
    gpsTimeSec.tv_sec += ( normGpsTime.ro * GPS_WEEKS_BEF_RO
                         + normGpsTime.wno ) * SEC_PER_WEEK;
...
{
    return modLeapSecond(unixTime, true);
}
...
{
    return modLeapSecond(unixTime, false);
}
...
bool isValidPos(POS_t const * pos)
{
    ...
    if( pos->latDeg >=MIN_LAT_DEG && pos->latDeg <= MAX_LAT_DEG
     && pos->lonDeg >=MIN_LON_DEG && pos->lonDeg <= MAX_LON_DEG
     && pos->posAccCm <= MAX_POS_INACC )
        result=true;
...
bool extractMgaIniTimeUtc(MGA_INI_TIME_UTC_t const * data, ACCTIME_t * accTime)
{
    ...
    tmpAccTime.time.tv_nsec=(iniPay[15]<<24)+(iniPay[14]<<16)+(iniPay[13]<<8)+iniPay[12];
                    ...
                    , POS_t const * pos /* = NULL */ )
{
    ...
    if (pos)
    {
            ...
            tmpPay.flags     |= GPS_UBX_AID_INI_U5__FLAGS_POS_MASK
    ...
    if(accTime)
    {
        ...
        if(timeGpsMs >= (uint64_t) GPS_EPOCH_SEC*MSEC_PER_SEC)
        {
            ...
            uint64_t accNs=((uint64_t)accTime->acc.tv_sec)*(uint64_t)NSEC_PER_SEC+(uint64_t)accTime->acc.tv_nsec;
