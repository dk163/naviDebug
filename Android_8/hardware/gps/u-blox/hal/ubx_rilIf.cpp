 ...
 *
 ******************************************************************************
 * $Id: ubx_rilIf.cpp 94373 2015-04-16 13:28:53Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/v3.30/gps/hal/ubx_rilIf.cpp $
 *****************************************************************************/
/*!
  \file
...
CRilIf::CRilIf()
{
	...
	pthread_mutex_init(&m_mutexNetwork, NULL);
    sem_init(&sem, 0, 0);
#if (PLATFORM_SDK_VERSION >= 14 /* =4.0 */)
	m_available = 0;
#else
...
void CRilIf::setRefLocation(const AGpsRefLocation *agps_reflocation, size_t sz_struct)
{
	if (agps_reflocation->type == AGPS_REG_LOCATION_TYPE_MAC)
		UBX_LOG(LCAT_VERBOSE, "size=%d type=%d(%s) -> mac=%02X-%02X-%02X-%02X-%02X-%02X",
		...
		UBX_LOG(LCAT_VERBOSE, "size=%d type=%d(%s) -> type=%d mcc=%d mnc=%d lac=%d cid=%d",
				sz_struct, agps_reflocation->type, _LOOKUPSTR(agps_reflocation->type, AGpsRefLocation),
		...
		UBX_LOG(LCAT_VERBOSE, "size=%d type=%d(%s)",
				sz_struct, agps_reflocation->type, _LOOKUPSTR(agps_reflocation->type, AGpsRefLocation));
...
{
	UBX_LOG(LCAT_VERBOSE, "connected=%d type=%d(%s) roaming=%d extra_info='%s'",
         connected,
...
{
	UBX_LOG(LCAT_VERBOSE, "available=%d apn='%s'",
         available,
         apn
        );
...
bool CRilIf::isConnected(void)
{
	bool result = false;
	pthread_mutex_lock(&m_mutexNetwork);
	if (m_available > 0 && m_networkType != RIL_NO_NETWORK)
	{
		result=true;
	}
	pthread_mutex_unlock(&m_mutexNetwork);
