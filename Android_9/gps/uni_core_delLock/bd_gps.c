...
static char productName[PROPERTY_VALUE_MAX] = {'\0'};
...
void gps_wakeup(GpsState *state)
{
	if( sleep_lock == 0) // it reset by um220_gps_start
	{
		...
		bGetFormalNMEA = 0;
		//gps_state_unlock_fix(state);//del
	}
}
...
static void gps_nmea_thread( void*  arg )
{
	...
	while (continue_thread)
	{
		...
		if(state->fd < 0)
		{   
		    DFR("gps nmea thread fd : %d, reopen device", state->fd);
		...
		if(ret > 0 && FD_ISSET(fd, &readfds) && is_valid_fd(fd)) {
			...
			else
			{
				...
				//gps_closetty(state);//del
				//GPS_STATUS_CB(state->callbacks, GPS_STATUS_SESSION_END);//del
...
int gps_opentty(GpsState *state)
{
	...
	if (!isSendConfig())
	{
		//state->callbacks.create_thread_cb("gps_opentty_thread", gps_opentty_thread, state);
		gps_opentty_thread(state);
		DFR("%s end", __FUNCTION__);
	}
	else
	{
		do {
...
static void gps_state_init( GpsState*  state )
{
	...
	//sprintf(productName, "%s", "chinatsp_s202_p_8q");//debug
	if (!isSendConfig())
	{
...
//	tcflush(fd, TCIOFLUSH);
    ...
    opt.c_iflag = IGNPAR;
    opt.c_oflag = 0;
    opt.c_lflag = 0;  /* disable ECHO, ICANON, etc... */
...
static void gps_dev_send(int fd, char *msg)
{
    ...
    if (!isSendConfig())
    {
        D("tty is USB,do not gps_dev_send");
        return;
    }
    //end
...
static void set_rnss_mode(int fd, char mode)
{
    ...
    if (!isSendConfig())
    {
        D("tty is USB,do not set_rnss_mode");
        return;
    }
    //end
...
static void gps_opentty_thread(void*  arg)
{
	...
	gettid(),getpid(),
	getuid(), geteuid());
	DFR("%s %d  state->fd = %d", __FUNCTION__, __LINE__,  state->fd); 
	do {
		i = 0;
		//try open ttyUSBn
		for(i = 0;i < 6;i++) {
			sprintf(tmp, "%s%d", GPS_READLINK_CMD, i);
			//DFR("readlink cmd str : %s ", tmp);
			...
			{
				DFR("readlink return: %s", data);
				sprintf(prop, "%s%d", GPS_DEVICE_TMP, i);
				DFR("check tty name : %s ", prop);

				if((strlen(data) > 0) && (strstr(data, GPS_USB_KEYWORD)   != NULL))
				{	
					DFR("readlink found gps tty name : %s ", prop);

					if(access(prop, R_OK)) {
						DFR("no permissions on errno = %d, (%s) ", errno, strerror(errno));
					}
					else
					{
						state->fd = open( prop, O_RDWR | O_NOCTTY | O_NONBLOCK);
				...
				{
					ALOGE("could not open gps serial device %s, errno = %d(%s), reopen  gps device , try times = %d", prop, errno, strerror(errno), try);
				}
			}
		...
		try++;
		DFR("gps device not exits, sleep 2s ,try open %d time", try);
		sleep(2);
	} while ((state->fd < 0) && (try <= TRY_TIMES)); 	

	if(state->fd < 0) {
		ALOGE("gps open device failed, try times = %d , return", TRY_TIMES);
	
		return;
	}
	else
	{
	    DFR("gps will read from %s", prop);
		...
		DFR("%s  state->fd = %d", __FUNCTION__, state->fd); 
		DFR("%s  _gps_state->fd = %d", __FUNCTION__, _gps_state->fd);
	}
}

//add
static int isSendConfig()
{
    //add
    if ((NULL != (strstr(productName, "mek_8q"))) || (NULL != strstr(productName, "chinatsp_s202_p_8q")))
    {
        D("tty is USB,do not isSendConfig");
        return 0;
    }
    else
    {
