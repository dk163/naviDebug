...
CParserBuffer::CParserBuffer()
{
  ...
  miSize = BUFFER_SIZE;
  // lint -e{1732,1733} new in constructor for class 'CParserBuffer' which has
  // no assignment/copy operator
...
bool CParserBuffer::Parse(CProtocol *&pProtocol, unsigned char *&pData, int &iSize)
{
  ...
  for (int iStart = miDone; (iStart < miUsed); iStart++)
  {
    ...
    while (pItem != nullptr)
    {
      ...
      iTemp = pItem->pProtocol->Parse(&mpBuffer[iStart], miUsed - iStart);
      if (iTemp == WAIT)
      {
        ...
        // TRACE(_T("CParserBuffer::Parse Buffer Overrun (skip one unsigned
        // char)\n"));
