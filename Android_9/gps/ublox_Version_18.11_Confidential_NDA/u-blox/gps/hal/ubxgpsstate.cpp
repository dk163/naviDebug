 ...
 * $HeadURL:
 *http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/v3.30/gps/hal/ubxgpsstate.cpp
 *$
 *****************************************************************************/
...
namespace
{
  ...
  static const unsigned int TIME_SOURCE = 0; //!< Default time source: system
  static const unsigned int SERPORT_BAUDRATE_DEFAULT =
    9600;                                                  //!< Default receiver interface baudrate
  static const unsigned int SHUTDOWN_TIMEOUT_DEFAULT = 10; //!< Default shutdown timeout: 10 seconds
  static const unsigned int AGNSS_OFFLINE_INTERVAL_DEFAULT =
    720; //!< Default AssistNow Offline download interval: 12 hours
  static const unsigned int AGNSS_ONLINE_INTERVAL_DEFAULT =
    120; //!< Default AssistNow Online download interval: 2 hours

#ifdef SUPL_ENABLED
  /* static const unsigned int MSA_RESPONSE_DELAY_DEFAULT =              10;
   * //!<
   * Default timeout (in seconds) to response with psedo ranges for MSA session
   */
  static const unsigned int NI_UI_TIMEOUT_DEFAULT =
    120; //!< Default timeout (in seconds) to display NI Notify/verify dialog
  static const unsigned int NI_RESPONSE_TIMEOUT =
    75; //!< Default timeout (in seconds) to respond to an NI request
#endif

  enum AGNSS_MODE : unsigned int
  {
    NONE = 0,               //!< No AGNSS enabled <- Default
    AOP_ONLY = 1,           //!< Only AssistNow Autonomous enabled
    OFFLINE_ONLY = 2,       //!< Only AssistNow Offline enabled
    ONLINE_ONLY = 3,        //!< Only AssistNow Online enabled
    AOP_AND_ONLINE = 4,     //!< AssistNow Autonomous and Online enabled
    OFFLINE_AND_ONLINE = 5, //!< AssistNow Offline and Online enabled
  };
  enum DB_FIELD : unsigned int
  {
    VERSION = 0,        //!< DB file location of the version information
    MEAS_RATE = 1,      //!< DB file location of the measurement rate
    POSITION = 2,       //!< DB file location of position information
    RESERVED0 = 3,      //!< DB file reserved field
    RESERVED1 = 4,      //!< DB file reserved field
    RESERVED2 = 5,      //!< DB file reserved field
    MGA_RECV_STATE = 6, //!< DB file location of MGA-DBD data
    LEG_RECV_STATE = 7, //!< DB file location of EPH and ALM data from the receiver
    RESERVED3 = 8,      //!< DB file reserved field
    RESERVED4 = 9,      //!< DB file reserved field
    MGA_OFFLINE = 10,   //!< DB file location of MGA Offline data
    LEG_OFFLINE = 11,   //!< DB file location of LEG Offline data
    RESERVED5 = 12,     //!< DB file reserved field
    RESERVED6 = 13,     //!< DB file reserved field
    RESERVED7 = 14,     //!< DB file reserved field
    LEG_AOP = 15,       //!< DB file location of LEG AOP data
    _FIELDCOUNT__ = 16  //!< Number of items in the DB files
  };
} // anonymous namespace
...
///////////////////////////////////////////////////////////////////////////////
//! Constructor
CUbxGpsState::CUbxGpsState() : m_timeLastOfflineReq{ getMonotonicMsCounter() }
{
  loadConfiguration();
  initializeAiding();
}
...
CUbxGpsState *CUbxGpsState::getInstance() { return &s_ubxGpsState; }

void CUbxGpsState::loadConfiguration()
{
  CCfg cfg;
...
void CUbxGpsState::loadConfiguration(const CCfg &cfg)
{

  m_pSerialDevice = cfg.get("SERIAL_DEVICE", Android::SERPORT_DEFAULT.c_str());
  ...
  m_pAgnssPersistenceFile = cfg.get("AGNSS_PERSISTENCE_FILE", Android::AIDING_DATA_FILE.c_str());
  m_timeSource = cfg.get("AGNSS_TIME_SOURCE", TIME_SOURCE);
  m_agnssOnlineInterval =
    cfg.get("AGNSS_ONLINE_INTERVAL", AGNSS_ONLINE_INTERVAL_DEFAULT) * 60 * 1000;
  m_agnssOfflineInterval =
    cfg.get("AGNSS_OFFLINE_INTERVAL", AGNSS_OFFLINE_INTERVAL_DEFAULT) * 60 * 1000;
  m_pAgnssOnlineServer1 = cfg.get("AGNSS_ONLINE_SERVER1", "");
  m_pAgnssOnlineServer2 = cfg.get("AGNSS_ONLINE_SERVER2", "");
  m_pAgnssOfflineServer1 = cfg.get("AGNSS_OFFLINE_SERVER1", "");
  m_pAgnssOfflineServer2 = cfg.get("AGNSS_OFFLINE_SERVER2", "");
  m_pAgnssToken = cfg.get("AGNSS_TOKEN", "");

  if (m_agnssOnlineInterval == 0)
  {
    m_agnssOnlineInterval = 60 * 1000;
    UBX_LOG(LCAT_WARNING,
            "Configuration value AGNSS_ONLILNE_INTERVAL too "
            "small! Set to the value 1");
  }
  ...
  if (m_agnssOfflineInterval == 0)
  {
    ...
    UBX_LOG(LCAT_WARNING,
            "Configuration value AGNSS_OFFLILNE_INTERVAL too "
            "small! Set to the value 1");
  }

#ifdef SUPL_ENABLED
  m_almanacRequest = static_cast<bool>(cfg.get("SUPL_ALMANAC_REQUEST", false));
  m_utcModelRequest = static_cast<bool>(cfg.get("SUPL_UTC_MODEL_REQUEST", false));
  m_ionosphericModelRequest = static_cast<bool>(cfg.get("SUPL_IONOSPHERIC_MODEL_REQUEST", false));
  m_dgpsCorrectionsRequest = static_cast<bool>(cfg.get("SUPL_DGPS_CORRECTIONS_REQUEST", false));
  m_refLocRequest = static_cast<bool>(cfg.get("SUPL_REF_LOC_REQUEST", false));
  m_refTimeRequest = static_cast<bool>(cfg.get("SUPL_REF_TIME_REQUEST", false));
  m_acquisitionAssistRequest = static_cast<bool>(cfg.get("SUPL_AQUISITION_ASSIST_REQUEST", false));
  m_realTimeIntegrityRequest = static_cast<bool>(cfg.get("SUPL_TIME_INTEGRITY_REQUEST", false));
  m_navigationModelRequest = static_cast<bool>(cfg.get("SUPL_NAVIGATIONAL_MODEL_REQUEST", false));
  m_fakePhone = static_cast<bool>(cfg.get("SUPL_FAKE_PHONE_CONNECTION", false));
  ...
  m_logSuplMessages = static_cast<bool>(cfg.get("SUPL_LOG_MESSAGES", false));
  m_cmccLogActive = static_cast<bool>(cfg.get("SUPL_CMCC_LOGGING", false));
  m_suplMsgToFile = static_cast<bool>(cfg.get("SUPL_MSG_TO_FILE", false));
#endif
  UBX_LOG(LCAT_VERBOSE,
          "Loaded standard configuration file %s, values:",
          Android::UBLOX_CONFIG_FILE.c_str());
  cfg.print();

  // inform the parser of the receiver generation (some messages may differ
  // from gen to gen).
  CProtocolUBX::setReceiverGeneration(m_receiverGeneration);
}

void CUbxGpsState::updateConfiguration(const CCfg &cfg)
{
  m_agnssStrategy = cfg.get("AGNSS_STRATEGY", m_agnssStrategy);
  m_agnssPersistence = cfg.get("AGNSS_PERSISTENCE", m_agnssPersistence);
  m_pAgnssPersistenceFile = cfg.get("AGNSS_PERSISTENCE_FILE", m_pAgnssPersistenceFile.c_str());
  m_timeSource = cfg.get("AGNSS_TIME_SOURCE", m_timeSource);
  m_agnssOnlineInterval = cfg.get("AGNSS_ONLINE_INTERVAL", m_agnssOnlineInterval);
  m_agnssOfflineInterval = cfg.get("AGNSS_OFFLINE_INTERVAL", m_agnssOfflineInterval);
  m_pAgnssOnlineServer1 = cfg.get("AGNSS_ONLINE_SERVER1", m_pAgnssOnlineServer1.c_str());
  m_pAgnssOnlineServer2 = cfg.get("AGNSS_ONLINE_SERVER2", m_pAgnssOnlineServer2.c_str());
  m_pAgnssOfflineServer1 = cfg.get("AGNSS_OFFLINE_SERVER1", m_pAgnssOfflineServer1.c_str());
  m_pAgnssOfflineServer2 = cfg.get("AGNSS_OFFLINE_SERVER2", m_pAgnssOfflineServer2.c_str());
  m_pAgnssToken = cfg.get("AGNSS_TOKEN", m_pAgnssToken.c_str());
...
#ifdef SUPL_ENABLED
  m_almanacRequest = static_cast<bool>(cfg.get("SUPL_ALMANAC_REQUEST", m_almanacRequest));
  m_utcModelRequest = static_cast<bool>(cfg.get("SUPL_UTC_MODEL_REQUEST", m_utcModelRequest));
  m_ionosphericModelRequest =
    static_cast<bool>(cfg.get("SUPL_IONOSPHERIC_MODEL_REQUEST", m_ionosphericModelRequest));
  m_dgpsCorrectionsRequest =
    static_cast<bool>(cfg.get("SUPL_DGPS_CORRECTIONS_REQUEST", m_dgpsCorrectionsRequest));
  m_refLocRequest = static_cast<bool>(cfg.get("SUPL_REF_LOC_REQUEST", m_refLocRequest));
  m_refTimeRequest = static_cast<bool>(cfg.get("SUPL_REF_TIME_REQUEST", m_refTimeRequest));
  m_acquisitionAssistRequest =
    static_cast<bool>(cfg.get("SUPL_AQUISITION_ASSIST_REQUEST", m_acquisitionAssistRequest));
  m_realTimeIntegrityRequest =
    static_cast<bool>(cfg.get("SUPL_TIME_INTEGRITY_REQUEST", m_realTimeIntegrityRequest));
  m_navigationModelRequest =
    static_cast<bool>(cfg.get("SUPL_NAVIGATIONAL_MODEL_REQUEST", m_navigationModelRequest));
  m_fakePhone = static_cast<bool>(cfg.get("SUPL_FAKE_PHONE_CONNECTION", m_fakePhone));
  m_niUiTimeout = cfg.get("SUPL_NI_UI_TIMEOUT", m_niUiTimeout);
  m_niResponseTimeout = cfg.get("SUPL_NI_RESPONSE_TIMEOUT", m_niResponseTimeout);
  m_logSuplMessages = static_cast<bool>(cfg.get("SUPL_LOG_MESSAGES", m_logSuplMessages));
  m_cmccLogActive = static_cast<bool>(cfg.get("SUPL_CMCC_LOGGING", m_cmccLogActive));
  m_suplMsgToFile = static_cast<bool>(cfg.get("SUPL_MSG_TO_FILE", m_suplMsgToFile));
#endif
}

void CUbxGpsState::initializeAiding()
{
  ...
  agnss_config.context = reinterpret_cast<void *>(this);
  destroyAgnssIf();
  if (getReceiverGeneration() <= 7) // Legacy interface
  {
    m_Db.pAgnssIf =
      std::unique_ptr<CAgnss>{ CAssistNowLeg::createInstance(m_pAgnssOnlineServer1.c_str(),
                                                             m_pAgnssOnlineServer2.c_str(),
                                                             m_pAgnssOfflineServer1.c_str(),
                                                             m_pAgnssOfflineServer2.c_str(),
                                                             getAgnssToken(),
                                                             &agnss_config) };
  }
  else // MGA interface
  {
    m_Db.pAgnssIf =
      std::unique_ptr<CAgnss>{ CAssistNowMga::createInstance(m_pAgnssOnlineServer1.c_str(),
                                                             m_pAgnssOnlineServer2.c_str(),
                                                             m_pAgnssOfflineServer1.c_str(),
                                                             m_pAgnssOfflineServer2.c_str(),
                                                             getAgnssToken(),
...
//! Lock this object
void CUbxGpsState::lock(void) { m_ubxStateMutex.lock(); }

//! Unlock this object
void CUbxGpsState::unlock(void) { m_ubxStateMutex.unlock(); }
...
void CUbxGpsState::init(CCfg &config)
{
  loadConfiguration(config);
  hardResetDevice();
}

void CUbxGpsState::updateConfig(CCfg &config)
{
  updateConfiguration(config);
  hardResetDevice();
}

void CUbxGpsState::hardResetDevice()
{
  setBaudRate();
  forceTurnOff();
...
bool CUbxGpsState::changeState(RECV_STATE_t targetState, bool transition)
{
  ...
  {
    UBX_LOG(LCAT_VERBOSE,
            "A receiver state change has been detected. Turn receiver %s!",
            targetState == ON ? "on" : "Off");
    switch (targetState)
    {
...
void CUbxGpsState::turnOn(void)
{
  ...
  writeUbxCfgMsg(0x01, 0x07); // enable UBX-NAV_PVT
  writeUbxCfgMsg(0xF0, 0x00); // enable NMEA-GGA
  writeUbxCfgMsg(0xF0, 0x01); // enable NMEA-GLL
  writeUbxCfgMsg(0xF0, 0x02); // enable NMEA-GSA
  writeUbxCfgMsg(0x02, 0x15); // enable UBX-RXM-RAWX
  writeUbxCfgMsg(0x02, 0x13); // enable UBX-RXM-SFRBX
  writeUbxCfgMsg(0x01, 0x22); // enable UBX-NAV-CLOCK
  ...
  {
    writeUbxCfgMsg(0x01, 0x35); // enable UBX-NAV-SAT
  }

#if (PLATFORM_SDK_VERSION >= 26 /* >=8.0 */)
  writeUbxCfgMsg(0x0A, 0x09); // enable UBX-MON-HW
#endif                        /* >=8.0 */
...
#if (PLATFORM_SDK_VERSION >= 26 /* >=8.0 */)
  writeUbxCfgMsg(0x0A, 0x09, false); // disable UBX-MON-HW
#endif                               /* >=8.0 */
...
*/
void CUbxGpsState::setTransferRequired(CAgnss::SERVICE_t service, bool required)
{
  if (service < m_transferRequired.size())
...
bool CUbxGpsState::isTransferPossibleAndRequired(CAgnss::SERVICE_t service)
{
  ...
  if (m_Db.pAgnssIf && service < m_transferRequired.size())
  {
    if ((service == CAgnss::TIME && m_Db.pAgnssIf->hasValidTime()) ||
        (service != CAgnss::TIME && m_Db.pAgnssIf->hasData(service)))
    {
...
*/
void CUbxGpsState::setCurrentChecksum(CAgnss::SERVICE_t service, uint16_t checksum)
{
  if (service < m_currentChecksum.size())
...
*/
bool CUbxGpsState::isCurrentChecksum(CAgnss::SERVICE_t service, uint16_t checksum)
{
  bool result = false;
  if (service < m_currentChecksum.size())
...
/*! With this function the database-has-changed-flag can be either set to true
    ...
    \param changed         : Set to true to indicate the database has changed
                             ...
                             changed
*/
void CUbxGpsState::setDbChanged(bool changed) { m_dbChanged = changed; }
...
*/
bool CUbxGpsState::hasDbChanged() { return m_dbChanged; }
  ...
  if (ril_if && ril_if->isConnected())
  {
    ...
    if (isOnlineEnabled())
    {
      ...
      if (!m_timeLastOnlineReq || (now - m_timeLastOnlineReq) >= m_agnssOnlineInterval)
      {
        ...
        {
          UBX_LOG(LCAT_WARNING,
                  "Could not initiate download of AssistNow "
                  "Online data. (Too many requests pending "
                  "already?)");
        }
      }
    }
...
void CUbxGpsState::initTransferIfReq()
{
  ...
  for (int i = 0; i < (int)CAgnss::_NUM_SERVICES_; ++i)
  {
    ...
    if (isTransferPossibleAndRequired(service))
    {
      ...
      {
        UBX_LOG(LCAT_WARNING,
                "Could not initiate transfer of %s. (Too many "
                "requests pending already?)",
...
/*! This is the main routine which is called whenever a new UBX message was
   received in the parser.

    \param pMsg     the pointer to the complete message (includes frameing and
   payload)
    \param iMsg     the size of the complete message (includes frameing and
   payload)
*/
void CUbxGpsState::onNewMsg(const unsigned char *pMsg, unsigned int iMsg)
{
  ...
  case OFF:
  {
    if (m_inTransition)
    {
      ...
      {
        UBX_LOG(LCAT_WARNING,
                "Time for preparing the shutdown run out. "
  ...
  if (isAopEnabled() && pMsg && iMsg > 4)
  {
    ...
    {
      unsigned int gnssId = pMsg[6];
      unsigned int svix = pMsg[7] - 1;
      if ((0x33 == msgId) && (iMsg > 9) && (gnssId == 0) && (svix < NUM_GPS_SVS))
      {
...
/*! Send a UBX-CFG-NAVX5 message to the receiver to enable the AssistNow
   Autonomous Feature.
...
*/
int64_t CUbxGpsState::currentRefTimeMs(void) { return getMonotonicMsCounter(); }

//! put time information into the local database.
/*! Put the time information received from a SNTP request into the local
   database.
...
void CUbxGpsState::putPos(double latDeg, double lonDeg, float accM)
{
  ...
  {
    UBX_LOG(LCAT_VERBOSE,
            "Retrieved the following position as the current "
            "from the framework: Lat %.6f Lon %.6f Acc %f",
            latDeg,
            lonDeg,
            (double)accM);
    POS_t pos{};
...
bool CUbxGpsState::putNewOfflineData(const unsigned char *pData, unsigned int iData)
{
  ...
  if (isOfflineEnabled())
  {
    ...
    if (m_Db.pAgnssIf->saveToDb(CAgnss::OFFLINE, pData, iData, &checksum))
    {
      ...
      {
        UBX_LOG(LCAT_VERBOSE,
                "Downloaded Offline data identical to already "
                "transferred one. Do nothing.");
      }
    }
    else
    {
      UBX_LOG(LCAT_ERROR,
              "Could not save Offline data to database, because "
              "the data is invalid");
    }
  }
  else
...
/*! write a UBX-CFGiMSG message to the receiver
    ...
    \return         true if sucessfull, false otherwise
*/
bool CUbxGpsState::writeUbxCfgMsg(unsigned char clsId,
                                  unsigned char msgId,
                                  bool enable /* = true */)
{
  GPS_UBX_CFG_MSG_SETCURRENT_t Payload{ clsId, msgId, 1 }; // default: enable

  // disable message instead of enable if required
  if (!enable)
  {
    Payload.rate = 0;
  }
  ...
  {
    UBX_LOG(LCAT_VERBOSE,
            "Sent CFG-MSG id=%02X-%02X (enable %s)",
            clsId,
            msgId,
            strUbxCfgMsg(clsId, msgId));
  }
  else
  {
    UBX_LOG(LCAT_WARNING,
            "Sending CFG-MSG id=%02X-%02X (enabling %s failed!)",
            clsId,
            msgId,
            strUbxCfgMsg(clsId, msgId));
  }
...
/*! write a UBX-CFG-RST message to the receiver
    ...
    \param resetMode    the resetMode to be executed (8=stop,9=start,2=gps
   reset)
    \param flags        the information to be cleared set to 0 if nothing needs
   to be cleared.
    \return             true if sucessfull, false otherwise
*/
bool CUbxGpsState::writeUbxCfgRst(int resetMode, GpsAidingData flags)
{
  ...
  else
  {
    if (flags & GPS_DELETE_EPHEMERIS)
      ubxFlags |= GPS_UBX_CFG_RST_NAVBBRMASK_EPH_MASK;
    if (flags & GPS_DELETE_ALMANAC)
      ubxFlags |= GPS_UBX_CFG_RST_NAVBBRMASK_ALM_MASK;
    if (flags & GPS_DELETE_POSITION)
      ubxFlags |= GPS_UBX_CFG_RST_NAVBBRMASK_POS_MASK;
    if (flags & GPS_DELETE_TIME)
      ubxFlags |= GPS_UBX_CFG_RST_NAVBBRMASK_RTC_MASK;
    if (flags & GPS_DELETE_IONO)
      ubxFlags |= GPS_UBX_CFG_RST_NAVBBRMASK_KLOB_MASK;
    if (flags & GPS_DELETE_UTC)
      ubxFlags |= GPS_UBX_CFG_RST_NAVBBRMASK_UTC_MASK;
    if (flags & GPS_DELETE_HEALTH)
      ubxFlags |= GPS_UBX_CFG_RST_NAVBBRMASK_HEALTH_MASK;
    if (flags & GPS_DELETE_SADATA)
      ubxFlags |= (1 << 15) /* AOP DATA */;
  }
...
{
  writeUbx(UBXID_AID_INI >> 8,
           UBXID_AID_INI & 0xFF,
           const_cast<GPS_UBX_AID_INI_U5__t *>(pAidingData),
           sizeof(GPS_UBX_AID_INI_U5__t));
#if 0
...
/*! Write a UBX message to the receiver with a specific class/message id and for
   a certain payload.

    \param classID  the class Id of the message to be sent
    \param msgID    the message Id of the message to be sent
    \param pData0   the pointer to the first part of the payload to be sent (can
   be NULL)
    \param iData0   the size of the first part of the payload to be sent (can be
   0)
    \param pData1   the pointer to the second part of the payload to be sent
   (can be NULL)
    \param iData1   the size of the second part of the payload to be sent (can
   be 0)
    \return         true if sucessfull, false otherwise
*/
bool CUbxGpsState::writeUbx(unsigned char classID,
                            unsigned char msgID,
                            const void *pData0,
                            size_t iData0,
                            const void *pData1,
                            size_t iData1)
{
  if (m_pSer == nullptr || ((!pData0 && !pData1) && (iData0 > 0 || iData1 > 0)))
  {
    return false;
  }
  ...
  if (m_pSer->writeSerial(outBuf, outCount) == outCount)
  {
    ...
    std::string message;
    message = byte_array_to_hex_string(outBuf, outCount);
    if (message.compare("") == 0)
    {
      UBX_LOG(LCAT_ERROR,
              "An error occured while trying to log outgoing UBX "
              "messages (part 2/2)");
    }
    else
    {
...
{
  return (m_agnssStrategy == AGNSS_MODE::AOP_ONLY || m_agnssStrategy == AGNSS_MODE::AOP_AND_ONLINE);
}
...
{
  return (m_Db.pAgnssIf && (m_agnssStrategy == AGNSS_MODE::OFFLINE_ONLY ||
                            m_agnssStrategy == AGNSS_MODE::OFFLINE_AND_ONLINE));
}
...
{
  return (m_Db.pAgnssIf && (m_agnssStrategy == AGNSS_MODE::ONLINE_ONLY ||
                            m_agnssStrategy == AGNSS_MODE::OFFLINE_AND_ONLINE ||
                            m_agnssStrategy == AGNSS_MODE::AOP_AND_ONLINE));
}
...
*/
int CUbxGpsState::printStd(const char *str) { return UBX_LOG(LCAT_VERBOSE, str); }
...
*/
int CUbxGpsState::printErr(const char *str) { return UBX_LOG(LCAT_ERROR, str); }
...
*/
ssize_t CUbxGpsState::writeToRcv(void const *context, unsigned char const *buf, size_t size)
{
  ssize_t result = -1;
  if (context)
  ...
  if (success)
  {
    ...
    case CAgnss::DOWNLOAD:
    {
      ...
      if (!isCurrentChecksum(service, dataId))
      {
        ...
        if (service == CAgnss::OFFLINE || service == CAgnss::RECV_AID_STATE)
        {
    ...
    {
      UBX_LOG(LCAT_VERBOSE,
              "Transfer of %s data has been successful!",
              (agnssServiceTypeToString(service)));
  ...
  else // failed
  {
    ...
    if (action == CAgnss::TRANSFER)
    {
      if (m_consFailedTransf < m_maxConsFailedTransf)
      {
        ++m_consFailedTransf;
        UBX_LOG(LCAT_VERBOSE,
                "Retry transfer. Maximum of consecutive failures "
                "while trying to transfer not yet reached. [Try "
                "%u/%u]",
                m_consFailedTransf,
                m_maxConsFailedTransf);
        setTransferRequired(service, true);
      }
      else
      {
        m_consFailedTransf = 0;
        UBX_LOG(LCAT_ERROR,
                "Maximum consecutive failures while trying to "
                "transfer reached (%u). Transfer will not be "
                "rescheduled",
                m_maxConsFailedTransf);
      }
    }
...
void CUbxGpsState::sendAopIfRequired()
{
  if (isAopEnabled())
  {
    for (int svix = 0; svix < NUM_GPS_SVS; svix++)
    {
      ...
      if ((p->p != nullptr) && (p->i > 0))
      {
        if ((m_pSer != nullptr) && (m_pSer->writeSerial(p->p, p->i) == (int)p->i))
        {
...
/*! Saves the current aiding data to a temporary file, ready to be loaded back
   into
...
void CUbxGpsState::saveAiding(void)
{
  ...
  {
    UBX_LOG(LCAT_VERBOSE, "Saving aiding data to %s", m_pAgnssPersistenceFile.c_str());

    mode_t mode = S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH;
    int fd = open(m_pAgnssPersistenceFile.c_str(), O_CREAT | O_WRONLY | O_TRUNC, mode);

    if (fd == -1)
    {
      // failed
      UBX_LOG(LCAT_VERBOSE,
              "Can not save aiding data to %s : %s",
              m_pAgnssPersistenceFile.c_str(),
              strerror(errno));
      return;
    }
    ...
    else
    {
      // Failed
      close(fd);
      remove(m_pAgnssPersistenceFile.c_str());
...
bool CUbxGpsState::loadAiding(void)
{
  if (m_agnssPersistence)
  {
    UBX_LOG(LCAT_VERBOSE, "Loading last aiding data from %s", m_pAgnssPersistenceFile.c_str());

    int fd = open(m_pAgnssPersistenceFile.c_str(), O_RDONLY);

    if (fd == -1)
    {
      // Failed
      UBX_LOG(LCAT_VERBOSE,
              "Can not load aiding data from %s : %i",
              m_pAgnssPersistenceFile.c_str(),
              errno);
      return false;
    }
...
void CUbxGpsState::destroyAgnssIf(void)
{
  if (m_Db.pAgnssIf)
  {
...
bool CUbxGpsState::saveDatabase(int fd, struct DB_s *pDb)
{
  ...
  ssize_t rawSize = 0;
  unsigned char *aopRaw = nullptr;
  ssize_t aopRawSize = 0;
  unsigned char *offlineData = nullptr;
  ssize_t offlineDataSize = 0;
  unsigned char *receiverStateData = nullptr;
  ssize_t receiverStateDataSize = 0;
  unsigned char *positionData = nullptr;
  ssize_t positionDataSize = 0;

  std::array<BUF_t, static_cast<unsigned int>(DB_FIELD::_FIELDCOUNT__)> data{};

  uint8_t dbVersion = DB_VERSION_NO;
  data[DB_FIELD::VERSION].p = (unsigned char *)&dbVersion;
  data[DB_FIELD::VERSION].i = sizeof(dbVersion);

  UBX_LOG(LCAT_VERBOSE, "Encoded Measurement rate!");
  data[DB_FIELD::MEAS_RATE].p = (unsigned char *)&(pDb->rateMs);
  data[DB_FIELD::MEAS_RATE].i = sizeof(&pDb->rateMs);
  ...
  if (m_Db.pAgnssIf)
  {
    ...
    {
      data[DB_FIELD::POSITION].p = positionData;
      data[DB_FIELD::POSITION].i = positionDataSize;
    ...
    {
      int db_receiverState = DB_FIELD::MGA_RECV_STATE;
      ...
      {
        db_receiverState = DB_FIELD::LEG_RECV_STATE;
    ...
    if (isOfflineEnabled())
    {
      ...
      {
        int db_offline = DB_FIELD::MGA_OFFLINE;
        if (getReceiverGeneration() >= 8)
        {
        ...
        {
          db_offline = DB_FIELD::LEG_OFFLINE;
  ...
  if (isAopEnabled())
  {
    ...
    if (aopRawSize > 0)
    {
      ...
      data[DB_FIELD::LEG_AOP].p = aopRaw;
      data[DB_FIELD::LEG_AOP].i = aopRawSize;
    }
  }

  bool result = false;
  UBX_LOG(LCAT_VERBOSE, "Create file structure for encoded data!");
  rawSize = encodeAgnssf00(&raw, data.data(), DB_FIELD::_FIELDCOUNT__);
  if (rawSize > 0)
  {
...
/*! The DB_s structure is populated one field at a time.
    To populate each field, a corresponding 'loadBuffer' function is called for
   handling that field type.
...
bool CUbxGpsState::loadDatabase(int fd, struct DB_s *pDb)
{
  ...
  auto raw = std::make_unique<unsigned char[]>(s.st_size);
  ...
  // Read the whole file
  if (read(fd, raw.get(), s.st_size) == s.st_size)
  {
    UBX_LOG(LCAT_VERBOSE, "Interpret file structure for encoded data!");
    BUF_t data[DB_FIELD::_FIELDCOUNT__];
    int8_t bufSize = decodeAgnssf00(raw.get(), s.st_size, data, DB_FIELD::_FIELDCOUNT__);
    if (bufSize <= 0)
    {
    ...
    {
      if (data[DB_FIELD::VERSION].i != sizeof(DB_VERSION_NO) ||
          memcmp(&DB_VERSION_NO, data[DB_FIELD::VERSION].p, sizeof(DB_VERSION_NO)))
      {
      ...
      {
        result = true;
        if (data[DB_FIELD::MEAS_RATE].i == sizeof(&pDb->rateMs))
        {
          memcpy(&(pDb->rateMs), data[DB_FIELD::MEAS_RATE].p, data[DB_FIELD::MEAS_RATE].i);
          UBX_LOG(LCAT_VERBOSE, "Decoded Measurement rate!");
        }
        else if (data[DB_FIELD::MEAS_RATE].i)
        {
        ...
        {
          // Load Position data
          BUF_t *positionData = &(data[DB_FIELD::POSITION]);

          if (positionData && positionData->p && positionData->i)
          {
            if (pDb->pAgnssIf->saveToDb(CAgnss::POSITION, positionData->p, positionData->i))
            {
              UBX_LOG(LCAT_VERBOSE, "Decoded and stored Position information!", sys.c_str());
            }
            else if (data[DB_FIELD::POSITION].i)
            {
          ...
          {
            if (pDb->pAgnssIf->saveToDb(
                  CAgnss::RECV_AID_STATE, receiverStateData->p, receiverStateData->i))
            {
              UBX_LOG(LCAT_VERBOSE,
                      "Decoded and stored %s Receiver Aiding State information!",
                      sys.c_str());
            }
            else
            {
              UBX_LOG(
                LCAT_WARNING, "Could not store %s Receiver Aiding State information!", sys.c_str());
            }
          }
          else
          {
            UBX_LOG(LCAT_VERBOSE,
                    "No %s Receiver Aiding State information found in the file",
                    sys.c_str());
          }
          ...
          if (isOfflineEnabled())
          {
            ...
            if (offlineData && offlineData->p && offlineData->i)
            {
              ...
              {
                UBX_LOG(LCAT_VERBOSE, "Decoded and stored %s Offline information!", sys.c_str());
              }
              else
              {
                UBX_LOG(LCAT_WARNING, "Could not store %s Offline information!", sys.c_str());
              }
            }
            else
            {
              UBX_LOG(LCAT_VERBOSE, "No %s Offline information found in the file", sys.c_str());
            }
          }
        }
        ...
        if (isAopEnabled())
        {
          if (data[DB_FIELD::LEG_AOP].p && data[DB_FIELD::LEG_AOP].i)
          {
            int8_t svs = decodeAgnssf00(
              data[DB_FIELD::LEG_AOP].p, data[DB_FIELD::LEG_AOP].i, pDb->aopG, NUM_GPS_SVS);
