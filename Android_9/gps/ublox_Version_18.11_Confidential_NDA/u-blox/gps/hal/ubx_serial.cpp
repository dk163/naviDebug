...
bool CSerialPort::openSerial(const char *pTty,
                             int ttybaud,
                             int blocksize,
                             TX_READY_CONF_t const *txReadyConf)
{
  UBXSERLOG(
    "CSerialPort::openSerial: pTty = %s, ttybaud = %d, blocksize = %d", pTty, ttybaud, blocksize);

  closeSerial(); // Make sure the serial interface is closed
  m_i2c = (strncmp(pTty, "/dev/i2c", 8) == 0);
  m_fd = open(pTty,
              (m_i2c ? 0 : O_NOCTTY) | O_RDWR
  ...
  {
    UBX_LOG(LCAT_ERROR,
            "Cannot open %s port %s, return %d: '%s'",
            m_i2c ? "i2c" : "serial",
            pTty,
            m_fd,
            strerror(errno));
  ...
  if (m_i2c)
  {
    ...
    UBX_LOG(LCAT_VERBOSE,
            "Configured driver to access the receiver over I2C "
            "%s on address 0x%02X",
            pTty,
            s_i2cRecvAddr);
    ...
    {
      UBX_LOG(LCAT_ERROR,
              "Writing the receiver register 0x%02X over I2C "
              "failed (return value: %i): %s",
              reg,
              reg_wr_res,
              strerror(errno));
      close(m_fd);
...
int CSerialPort::rselect(int maxFd, fd_set *rfds, struct timeval const *timeout)
{
  ...
  if (m_i2c)
  {
    ...
    unsigned long maxWakeups =
      i2c_poll_freq_hz * timeout->tv_sec + (i2c_poll_freq_hz * timeout->tv_usec) / 1000000;

    // If timeout->tv_sec and timeout->tv_usec is 0, iterate through the loop
    // below
  ...
  {
    struct timeval currInterval;
    currInterval = *timeout;
    res = select(maxFd, rfds, NULL, NULL, &currInterval);
  }
  return res;
}

int CSerialPort::setbaudrate(int ttybaud) { return settermios(ttybaud, -1); }
...
int CSerialPort::changeBaudrate(char *pTty, int *pBaudrate, const unsigned char *pBuf, int length)
{
  ...
  if (pBuf[5] != 0x00)
    return 0;
  if ((pBuf[6] != 0x01) && (pBuf[6] != 0xff))
    return 0;

  std::copy(pBuf + 8 + 6, pBuf + 8 + 6 + 4, &newbaudrate);
  ...
  if (newbaudrate != (unsigned long)*pBaudrate)
  {
    ...
    UBX_LOG(
      LCAT_WARNING, "Attempting to change baudrate from %d to %lu on %s)", tmp, newbaudrate, pTty);
...
int CSerialPort::retrieveErrors() const
{
  ...
  {
    /* cannot read the counters! ignore, as we might be talking to a device
     * without error counters */
  }
#endif
  return res;
}
...
bool CSerialPort::i2cIsTxReadySet(void)
{
  ...
  if (m_txReadyConf.enabled)
  {
    ...
    {
      UBX_LOG(LCAT_ERROR,
              "I2C: An invalid value was detected as TX-Ready "
              "state. Disabling TX-Ready.");
      m_txReadyConf = {};
      result = true;
      break;
    }
...
#endif // defined (UBX_SERIAL_EXTENSIVE_LOGGING_W) || defined
       // (UBX_SERIAL_EXTENSIVE_LOGGING_R)
...
ssize_t CSerialPort::writeSerial(const void *pBuffer, size_t size) const
{
  ...
  auto p = std::make_unique<unsigned char[]>(size + 1);
  if (!p)
    return 0;
  p[0] = 0xFF; // allways address the stream register
  if (!pBuffer)
    return 0;
  std::copy(reinterpret_cast<const unsigned char *>(pBuffer),
            reinterpret_cast<const unsigned char *>(pBuffer) + size,
            &(p[1]));
  ssize_t written = write(m_fd, p.get(), size + 1);
...
bool CSerialPort::isFdOpen(void) const { return m_fd > 0; }
...
void CSerialPort::i2cConfigTxReady(TX_READY_CONF_t const *txReadyConf)
{
  if (txReadyConf && txReadyConf->enabled)
  {
    if (i2cCheckGpioValue(txReadyConf->hostGpio) >= 0)
    {
      ...
      {
        // Send B5 62 06 00 14 00 00 00 B5 00 84 00 00 00 00 00 00 00 07 00 03
        // 00 00 00 00 00 5D 8C
        UBX_LOG(LCAT_VERBOSE,
                "Setting I2C TX-Ready PIO on the receiver to %i "
                "and check the host GPIO to %i ",
                txReadyConf->recvPio,
                txReadyConf->hostGpio);

        unsigned char cfgPrtI2cPayload[] = {0x00, 0x00, 0x81, 0x00, 0x84, 0x00, 0x00,
        ...
        if (pktSize > 0)
        {
          ...
          if (writtenSize == pktSize)
          {
            ...
            m_txReadyConf = *txReadyConf;
          }
          else
          {
            UBX_LOG(LCAT_ERROR,
                    "An error occured while trying to configure the receiver "
                    "to use TX-Ready!"
        ...
        {
          UBX_LOG(LCAT_ERROR,
                  "An error occured while trying to create the configuration "
                  "packet for TX-Ready!"
                  " Disable I2C TX-Ready feature and use polling instead.");
        }
      }
      ...
      {
        UBX_LOG(LCAT_ERROR,
                "The provided PIO does not exist or can not"
    ...
    {
      UBX_LOG(LCAT_ERROR,
              "The provided GPIO does not exist or can not"
