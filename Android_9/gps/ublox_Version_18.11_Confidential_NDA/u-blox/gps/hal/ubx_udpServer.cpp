...
int CUdpServer::recvPort(char *pBuf, int buflen)
{
  ...
  int conn;
  int len = recvfrom(
    m_fd, pBuf, (unsigned int)buflen, 0, (struct sockaddr *)(void *)&si_other, (socklen_t *)&slen);
  ...
  if (conn == -1) // not known connection
  {
    ...
    if ((len == 3) && (pBuf[0] == UDP_MUX_PACKET_HEADER) && (pBuf[1] == UDP_MUX_START_SESSION))
    {
      ...
      else
      {
        ...
        UBX_LOG(LCAT_VERBOSE,
                "new connection %i (%s:%i), feature 0x%.2X",
                conn,
        ...
        /* Send the notification to the clients that has requested to be
         * informed */
        int a = activeConnections();
        unsigned char buf[4] = {
          'X', UDP_MUX_NUM_CLIENT, (unsigned char)(a & 0xFF), (unsigned char)((a >> 8) & 0xFF)
        };
    ...
    else
    {
      ...
      UBX_LOG(LCAT_WARNING,
              "The message has not been recognized: len = %d, code = %d",
              len,
              (len > 0 ? pBuf[0] : 0));
    }
  ...
  m_udpConn[conn].seen = getMonotonicMsCounter(); // update timeout data, as we
                                                  // have received data from
                                                  // this client
  if (len > 0)
  {
...
#if !defined(_lint)
        UBX_LOG(LCAT_VERBOSE,
                "connection renegotiation %i (%s:%i), feature 0x%.2X",
                conn,
                inet_ntoa(m_udpConn[conn].conn.sin_addr),
                ntohs(m_udpConn[conn].conn.sin_port),
                m_udpConn[conn].flags);
    ...
    for (conn_1 = 0; conn_1 < MAXUDPCONN; conn_1++)
    {
      ...
      if ((m_udpConn[conn_1].conn.sin_port) && (conn_1 != conn) &&
          (m_udpConn[conn_1].flags & CLIENT_TRASPARENT_MODE))
      {
        ssize_t res;
        res = sendto(m_fd,
                     pBuf,
...
void CUdpServer::sendPort(const unsigned char *pBuf, int len) const
{
  ...
  for (conn = 0; conn < MAXUDPCONN; conn++)
  {
    if (m_udpConn[conn].conn.sin_port)
    {
      ...
      {
        // UBX_LOG(LCAT_VERBOSE, "Nothing sent. Client on %i not showing any
        // interest (0x%.2X) ", conn, m_udpConn[conn].flags);
...
#if !defined(_lint)
        UBX_LOG(LCAT_VERBOSE,
                "timeout on connection %i (%s:%i): %lld\n",
                conn,
                inet_ntoa(m_udpConn[conn].conn.sin_addr),
                ntohs(m_udpConn[conn].conn.sin_port),
