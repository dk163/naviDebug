...
const GpsXtraInterface CXtraIf::s_interface = {
  IF_ANDROID23(.size = sizeof(GpsXtraInterface), ).init = CXtraIf::init,
...
int CXtraIf::init(GpsXtraCallbacks *callbacks)
{
  ...
  s_myIf.m_ready = true;
  return 0;
  // lint -e{818} remove Pointer parameter 'callbacks' (line 46) could be
...
int CXtraIf::injectData(char *data, int length)
{
  ...
  return 0;
  // lint -e{818} remove Pointer parameter 'data' could be declared as pointing
  // to const
}
