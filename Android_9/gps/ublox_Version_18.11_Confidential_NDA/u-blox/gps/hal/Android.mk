...
#if using HIDL, we need to build the service which will launch the driver
ifeq ($(USE_HIDL), true)

include $(CLEAR_VARS)
LOCAL_MODULE_RELATIVE_PATH := hw
LOCAL_MODULE := android.hardware.gnss@1.0-service-ubx
LOCAL_INIT_RC := hidl/android.hardware.gnss@1.0-service-ubx.rc
LOCAL_PROPRIETARY_MODULE := true
LOCAL_SRC_FILES := \
    hidl/service.cpp
...
LOCAL_SHARED_LIBRARIES := \
    ...
    libhidltransport \
    android.hardware.gnss@1.0 \
    android.hardware.gnss@1.0-impl-ubx

include $(BUILD_EXECUTABLE)

endif # USE_HIDL

# copy u-blox.conf in /(vendor|system)/etc
include $(CLEAR_VARS)
LOCAL_MODULE := u-blox.conf
LOCAL_MODULE_CLASS := ETC
  LOCAL_PROPRIETARY_MODULE := true
LOCAL_PREBUILT_MODULE_FILE := $(LOCAL_PATH)/../u-blox.conf
