...
private:                                    // Variables
  ...
  static const UBX_MSG_TYPE _allowed[];     //! Allowed UBX messages for clients to send to receiver
  static const unsigned char _passPhrase[]; //! Passphrase for authentication
  std::list<AUTH_SOURCE_t> _authList;       //! Is a source authenticated?
  std::mutex _authMutex;                    //! Mutex to protect _authList
};
