    ...
    namespace gnss
    {
      namespace V1_0
      {
        namespace implementation
        {
          ...
          AGpsCallbacks AGnss::sAGnssCb = {.status_cb = statusCb,
     ...
     */
            ...
            if (status->size == sizeof(AGpsStatus))
            {
              ...
              case AF_INET6:
              {
                ...
                aGnssStatusIpV6.status =
                  static_cast<IAGnssCallback::AGnssStatusValue>(status->status);

                struct sockaddr_in6 *in6 = reinterpret_cast<struct sockaddr_in6 *>(&(status->addr));
                memcpy(&(aGnssStatusIpV6.ipV6Addr[0]),
 ...
 */
                                        ...
                                        int32_t port)
          {
            ...
            return (mAGnssIface->set_server(static_cast<AGpsType>(type), hostname.c_str(), port) ==
                    0);
          }
          ...
          Return<bool> AGnss::dataConnOpen(const hidl_string &apn, IAGnss::ApnIpType apnIpType)
          {
            ...
            return (mAGnssIface->data_conn_open_with_apn_ip_type(
                      apn.c_str(), static_cast<uint16_t>(apnIpType)) == 0);
          }
