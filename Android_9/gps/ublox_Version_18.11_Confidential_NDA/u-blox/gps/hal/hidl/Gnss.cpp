    ...
    namespace gnss
    {
      namespace V1_0
      {
        namespace implementation
        {
          ...
          Gnss::Gnss() : mDeathRecipient(new GnssHidlDeathRecipient(this))
          {
            /* Error out if an instance of the interface already exists. */
            LOG_ALWAYS_FATAL_IF(sInterfaceExists);
            sInterfaceExists = true;

            mGnssIface = CGpsIf::getIf(nullptr);
          }
          ...
          void Gnss::locationCb(GpsLocation *location)
          {
            ...
            android::hardware::gnss::V1_0::GnssLocation gnssLocation =
              convertToGnssLocation(location);
            auto ret = sGnssCbIface->gnssLocationCb(gnssLocation);
            if (!ret.isOk())
            {
              std::string isObjDeadStr = ((ret.isDeadObject() == true) ? "Yes" : "No");
              ALOGE("%s: Unable to invoke callback! Is object dead? \"%s\" Reason: \"%s\"",
                    __func__,
                    isObjDeadStr.c_str(),
     ...
     */
         ...
         */
          ...
          void Gnss::nmeaCb(GpsUtcTime timestamp, const char *nmea, int length)
          {
            ...
            // It is necessary to create single, 0-terminated NMEA sentences like this,
            // because the Android Framework is not respecting the 'length' field for
            // some reason. This might be a bug in the Android Framework.
            std::string singleNmeaSentence(nmea, length);

            android::hardware::hidl_string nmeaString;
            nmeaString.setToExternal(singleNmeaSentence.c_str(), singleNmeaSentence.size());
          ...
          void Gnss::acquireWakelockCb() { acquireWakelockGnss(); }

          void Gnss::releaseWakelockCb() { releaseWakelockGnss(); }
          ...
          void Gnss::updateWakelock()
          {
            ...
            if (sWakelockHeldGnss || sWakelockHeldFused)
            {
              if (!sWakelockHeld)
              {
                ALOGI("%s: GNSS HAL Wakelock acquired due to gps: %d, fused: %d",
                      __func__,
                      sWakelockHeldGnss,
                      sWakelockHeldFused);
                sWakelockHeld = true;
          ...
          void Gnss::setSystemInfoCb(const LegacyGnssSystemInfo *info)
          {
            ...
            IGnssCallback::GnssSystemInfo gnssInfo = {.yearOfHw = info->year_of_hw };
                                            ...
                                            float accuracyMeters)
          {
            ...
            return (
              mGnssIface->inject_location(latitudeDegrees, longitudeDegrees, accuracyMeters) == 0);
          }

          Return<bool>
          Gnss::injectTime(int64_t timeMs, int64_t timeReferenceMs, int32_t uncertaintyMs)
          {
            if (mGnssIface == nullptr)
            {
              ALOGE("%s: Gnss interface is unavailable", __func__);
          ...
          Return<sp<IAGnss>> Gnss::getExtensionAGnss()
          {
            ...
            {
              const AGpsInterface *agpsIface =
                static_cast<const AGpsInterface *>(mGnssIface->get_extension(AGPS_INTERFACE));
          ...
          Return<sp<IGnssNi>> Gnss::getExtensionGnssNi()
          {
            ...
            if (mGnssNi == nullptr)
            {
              const GpsNiInterface *gpsNiIface =
                static_cast<const GpsNiInterface *>(mGnssIface->get_extension(GPS_NI_INTERFACE));
          ...
          Return<sp<IGnssNavigationMessage>> Gnss::getExtensionGnssNavigationMessage()
          {
            ...
            if (mGnssNavigationMessage == nullptr)
            {
              ...
              {
                ALOGE("%s GnssNavigationMessage interface not implemented by GNSS HAL", __func__);
              }
              else
              {
          ...
          Return<sp<IGnssBatching>> Gnss::getExtensionGnssBatching()
          {
            ...
            if (mGnssBatching == nullptr)
            {
              ...
              int err =
                hw_get_module(FUSED_LOCATION_HARDWARE_MODULE_ID, (hw_module_t const **)&module);
