      ...
      namespace V1_0
      {
        namespace implementation
        {
          ...
          void GnssMeasurement::gnssMeasurementCb(LegacyGnssData *legacyGnssData)
          {
            ...
            gnssData.measurementCount =
              std::min(legacyGnssData->measurement_count, static_cast<size_t>(GnssMax::SVS_COUNT));

            for (size_t i = 0; i < gnssData.measurementCount; i++)
            {
              auto entry = legacyGnssData->measurements[i];
              auto entryExt = ((LegacyGnssDataExt *)legacyGnssData)->measurements[i];
              ...
              gnssData.measurements[i] = {
                ...
                .snrDb = entry.snr_db,
                .agcLevelDb = entryExt.agc_level_db,
              };
            }
 ...
 */
          void GnssMeasurement::gpsMeasurementCb(GpsData *gpsData)
          {
            ...
            IGnssMeasurementCallback::GnssData gnssData;
            gnssData.measurementCount =
              std::min(gpsData->measurement_count, static_cast<size_t>(GnssMax::SVS_COUNT));
            ...
            for (size_t i = 0; i < gnssData.measurementCount; i++)
            {
              ...
              {
                gnssData.measurements[i].constellation = GnssConstellationType::UNKNOWN;
              }
              ...
              gnssData.measurements[i].accumulatedDeltaRangeM = entry.accumulated_delta_range_m;
     ...
     */
          ...
          Return<GnssMeasurement::GnssMeasurementStatus>
          GnssMeasurement::setCallback(const sp<IGnssMeasurementCallback> &callback)
          {
