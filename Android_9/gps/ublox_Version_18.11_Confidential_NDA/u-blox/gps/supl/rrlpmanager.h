...
typedef struct
{
  ...
  int referenceNumber;         //!< RRLP session reference number to be given in the
                               //!answer
  int reqAccuracy;             //!< Requested horizontal accuracy (in cm)
  RrlpRespData_t responseType; //!< Type of response needed
  //	bool assistDataReceived;		//!< Flag if assistance data has been
  //received
} aux_t;
...
char *buildRrlpPosResp(
  int *pOutSize, int referenceNumber, double lat, double lon, double alt, double tow);
