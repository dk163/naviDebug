...
private:
  ...
  TRI_STATE_FUNC_RESULT_t continueMgaTransfer(unsigned char const *buf, size_t size);
  ...
  TRI_STATE_FUNC_RESULT_t getEnabledGnssFromRcv(unsigned char const *buf, size_t size);
  ...
  static const struct UBX_MSG_TYPE _online_allowed[];  //!< Valid MGA Online UBX msgs
  static const struct UBX_MSG_TYPE _offline_allowed[]; //!< Valid MGA Offline UBX msgs
