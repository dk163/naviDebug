...
public: // Definitions
  ...
  /*! Pointer to a function that processes data read by \ref CComThread
    ...
    */
  typedef size_t (*COM_PROCESS_p)(void const *context,
                                  size_t source,
                                  unsigned char *buf,
                                  size_t size);
...
private:                 // Variables
  ...
  std::unique_ptr<std::thread> pComThread; //!< Reference to the object internal thread

  bool _isStarted;              //!< Indicates if \ref start was succesful
  std::mutex _isStartedMutex{}; //!< Avoid _isStarted race conditions

  bool _runFlag;              //!< Indicates if the thread is running
  std::mutex _runFlagMutex{}; //!< Avoid _fds and _numFds race conditions

  std::list<FD_POS_t> _fds; //!< File descriptor list
  std::mutex _fdsMutex{};   //!< Avoid _fds race conditions

  std::list<FD_POS_t> _dAddFds;       //!< File descriptors to be added to the list
  std::list<size_t> _dCloseFds;       //!< File descriptors to be removed from the list
  std::list<W_DELAYED_t> _dWriteBufs; //!< Data to write to the position delayed
  std::mutex _dMutex{};               //!< Avoid race conditions for delayed (_d*) operations

  size_t _numFds;            //!< Number of file descriptors since creation
  std::mutex _numFdsMutex{}; //!< Avoid _fds race conditions
};
