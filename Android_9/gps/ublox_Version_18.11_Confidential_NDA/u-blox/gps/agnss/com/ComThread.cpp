...
/*! The constructor creates an object of this class and will initialise it to
    ...
    called successfully.
*/
CComThread::CComThread() : _config(NULL), _isStarted(false), _runFlag(false), _fds(), _numFds(0) {}

/*! The destructor will teardown the object and free the ressources
    where required.
*/
CComThread::~CComThread() { stop_no_check(); }
...
ssize_t CComThread::writeToSync(size_t dest, unsigned char const *buf, size_t size)
{
  ...
  ssize_t result = -1;
  {
    ...
    for (auto i : _fds)
    {
      ...
      if (i.pos == dest || (dest > SSIZE_MAX && i.pos + SSIZE_MAX + 1 >= dest))
      {
        ssize_t writtenSize = write(i.fd, buf, size);
...
int CComThread::remove(size_t pos)
{
  ...
  int result = -1;
  {
    ...
    std::lock_guard<std::mutex> lock(_fdsMutex);
    for (auto i = _fds.begin(); (i != _fds.end()) && result == -1;)
    {
      if (i->pos == pos)
      {
        if (i->buf != nullptr)
...
bool CComThread::readAndProcess()
{
  ...
  else // There is data input
  {
    ...
    for (size_t i = 0; i < arrFd.num; ++i)
    {
      ...
      else if ((revents & POLLIN) && notify(pos, DATA_AVAILABLE))
      {
...
bool CComThread::handleDelayedAdd()
{
  ...
  FD_POS_t tmpFd{};
  while (_dAddFds.size() > 0)
  {
    std::lock_guard<std::mutex> lock(_fdsMutex);
    std::lock_guard<std::mutex> lock2(_dMutex);
    //check no one tampered with _dAddFds in the meantime
    if(_dAddFds.size() <= 0)
        continue;
    tmpFd = _dAddFds.front();
    _fds.push_back(tmpFd);
    result = true;
    _dAddFds.pop_front();
  }
  return result;
...
bool CComThread::handleDelayedClose()
{
  ...
  size_t pos;
  while (_dCloseFds.size() > 0)
  {
      std::lock_guard<std::mutex> lock(_dMutex);
      //check no one tampered with _dCloseFds in the meantime
      if(_dCloseFds.size() <= 0)
          continue;

    pos = _dCloseFds.front();
    result = true;
    int fd = remove(pos);
    assert(fd > 0);
    if (fd > 0)
...
bool CComThread::handleDelayedWrite()
{
  assert(isStarted());
  bool result = false;
  while (_dWriteBufs.size() > 0)
  {
    W_DELAYED_t tmp;
    std::lock_guard<std::mutex> lock(_dMutex);
    //check no one tampered with _dWriteBufs in the meantime
    if(_dWriteBufs.size() <= 0)
        continue;
    tmp = _dWriteBufs.front();
    result = true;
    writeToSync(tmp.dest, tmp.buf, tmp.size);
    free(tmp.buf);
    _dWriteBufs.pop_front();
  }
  return result;
}
  ...
  if (size > 0)
  {
    ...
    if (pollFdTmp && posTmp)
    {
      ...
      for (auto i : _fds)
      {
        ...
        pollFdTmp[counter].fd = i.fd;
        posTmp[counter] = i.pos;
        ++counter;
      }
      assert(counter == size);
...
{
  if (isStarted() || !config ||
      !config->func_process) // Can't open as resource is already is opened
    return false;
  ...
  pComThread = std::make_unique<std::thread>(CComThread::bootstrapThread, this);
  //everything is fine
...
ssize_t CComThread::writeTo(size_t *dest, size_t count, unsigned char const *buf, size_t size)
{
  ...
  {
    if (dest[i] <= (size_t)SSIZE_MAX && (size_t)writeTo(dest[i], buf, size) == size)
      ++numFdsWritten;
  }
...
void CComThread::stop_no_check()
{
  ...
  {
    std::lock_guard<std::mutex> lock(_fdsMutex);
    //check no one tampered with _fds in the meantime
    if(_fds.size() <= 0)
        continue;
    tmpFd = _fds.front();
    if (tmpFd.fd > 0)
...

  {
      ...
      auto iter = _dWriteBufs.begin();
      while (iter != _dWriteBufs.end())
      {
          free(iter->buf);
          iter->buf = nullptr;
          iter = _dWriteBufs.erase(iter);
      }
  ...
  {
    std::lock_guard<std::mutex> lock(_numFdsMutex);
    _numFds = 0;
  }
}
...
{
  std::lock_guard<std::mutex> lock(_runFlagMutex);
  _runFlag = newValue;
}
...
{
  std::lock_guard<std::mutex> lock(_isStartedMutex);
  _isStarted = newValue;
}
