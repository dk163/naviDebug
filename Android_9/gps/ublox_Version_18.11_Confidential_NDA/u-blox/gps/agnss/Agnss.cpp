...
namespace
{
  bool isExecutingAction(const std::list<CFuncMngr> &executionList)
  {
    return executionList.size() > 0;
  }

  bool haveActionAvailable(const std::list<CAgnss::SERVICE_ACTION_PAIR_t> &actions)
  {
  ...
  : CThreadedPacketIf(), _time(_data[TIME]), _pos(_data[POSITION], _time)
{
  ...
  _print_prefix = std::vector<char>(print_prefix, print_prefix + std::strlen(print_prefix) + 1);
}
...
{
  bool result = false;
  if (!isOpen() && _conf && havePrintPrefix())
  {
    _fd = this->open();
    result = (_fd >= 0);
  }
...
/*! This function will pass the data passed to the thread for
    ...
    \return             The number of bytes written on success
                        and -1 on failure.
*/ ssize_t
CAgnss::processMsg(unsigned char const *buf, size_t size)
{
  ssize_t result = -1;
  if (isOpen() && buf && size)
  {
...
bool CAgnss::scheduleAction(SERVICE_t service, ACTION_t action)
{
  ...
  if (result)
  {
    print_std("Successfully scheduled %s of %s data",
              agnssActionToString(action),
              agnssServiceTypeToString(service));
  }
  else
  {
    print_err("Could not schedule %s of %s data",
              agnssActionToString(action),
              agnssServiceTypeToString(service));
  }
  return result;
...
*/
ssize_t CAgnss::loadFromDb(SERVICE_t service, unsigned char **data, uint16_t *const data_id)
{
  return _data[service].getCopy(data, data_id);
}

bool CAgnss::hasData(SERVICE_t service) { return _data[service].hasData(); }
...
{
  bool result = false;
  if (_conf && _conf->time_source == TIME_INTERNAL && accNow && accNow->valid)
  {
    ACCTIME_t tmpAccNow;
...
*/
bool CAgnss::hasValidTime() { return getCurrentTime(NULL); }
...
/*! Set the current position
    ...
    \return             true on success and false otherwise
*/
bool CAgnss::setCurrentPosition(POS_t const *here) { return _pos.injectPosition(here); }
...
/*! Get the current position
    ...
    \return             true on success and false otherwise
*/
bool CAgnss::getCurrentPosition(POS_t *here) { return _pos.getCurrentPosition(here); }
...
/*! This static function will be called by used libaries to write data to
    ...
    \return
*/
ssize_t CAgnss::writeToRcv(const void *context, unsigned char const *buf, size_t size)
{
  ssize_t result = -1;
  if (context)
  {
...
*/
int CAgnss::impl_close() { return impl_deinit() ? 0 : -1; }
...
*/
void CAgnss::impl_timeout() { impl_process(0, NULL, 0); }
...
int CAgnss::printWithFunc(PRINT_FUNC_p func, const char *format, va_list va)
{
  ...
  if (func && _conf)
  {
    ...
    if (havePrintPrefix())
    {
      res = asprintf(&new_format, "%s%s", _print_prefix.data(), format);
    }
    if (res >= 0)
    {
...
void CAgnss::stateCheck(unsigned char *buf, size_t size)
{
  ...
  ////////////////////
  {
    std::lock_guard<std::mutex> lock(_req_mutex);
    // Is there still something to do for a
    // current job?
    if (!isExecutingAction(_exec_list))
    {
      // No. Get a new one then if one is available
      if (haveActionAvailable(_req_action))
      {
        curr = _req_action.front();
        // Get new commands
  ...
  if (action_finished)
  {
    if (success)
    {
      print_std("%s %s finished successfully!",
                agnssServiceTypeToString(curr.service),
                agnssActionToString(curr.action));
    }
    else
    {
      print_err("%s %s finished unsuccessfully! Action failed!",
                agnssServiceTypeToString(curr.service),
                agnssActionToString(curr.action));
    }
    _conf->func_transfer_finished(
      (void const *)_conf->context, curr.service, curr.action, success, impl_getDataId());
  }
}
...
bool CAgnss::havePrintPrefix() const
{
  //more than just default \n in vector
  return _print_prefix.size() > 1;
}
