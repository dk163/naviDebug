...
////////////////////////////////////////////////////////////////////////////////
// Definitions & Types
#define MAX_UDP_PACKET_LEN                                                                         \
  16384                  //!< Dimension of the temporary buffer for reading a complete UDP packet
...
/*!
  ...
  \param result				: Last command result to make available
...
static void signal_cmd_complete(ControlThreadInfo *pControlThreadInfo, int result)
{
  ...
  {
    std::lock_guard<std::mutex> guard(pControlThreadInfo->threadCmdCompleteMutex);
    pControlThreadInfo->cmdResult = result;
  }
  pControlThreadInfo->threadCmdCompleteCond.notify_all();
  ...
  {
    UBX_LOG(LCAT_DEBUG,
            "Wait for ubx-main to signalise that everything is "
            "ready for normal operation");
    std::unique_lock<std::mutex> lock(pControlThreadInfo->threadMainReadyMutex);
    pControlThreadInfo->threadMainReadyCond.wait(
      lock, [&pControlThreadInfo] { return pControlThreadInfo->mainReady; });
    lock.unlock();
...
static void engineStop(ControlThreadInfo *pControlThreadInfo, bool si)
{
  ...
  pControlThreadInfo->threadDataAccessMutex.lock();
  ...
  pControlThreadInfo->threadDataAccessMutex.unlock();
...
static void handle_init(ControlThreadInfo *pControlThreadInfo)
{
  ...
  // Signal main
  UBX_LOG(
    LCAT_VERBOSE, "(%u): Init state. Signal main-thread to continue", (unsigned int)pthread_self());
    ...
    if (si)
    {
      ...
      if (suplActiveSessions())
      {
        UBX_LOG(LCAT_VERBOSE,
                "There are already active Set Initiated SUPL Sessions (%s)!",
                _LOOKUPSTR(posMode, GpsPositionMode));
      }
      else
      {
        ...
        {
          UBX_LOG(LCAT_VERBOSE,
                  "Start Set Initiated Action for the %s SUPL mode!",
                  _LOOKUPSTR(posMode, GpsPositionMode));
        ...
        {
          UBX_LOG(LCAT_VERBOSE,
                  "Set Initiated Action for this SUPL mode (%s) "
                  "will not be started!",
                  _LOOKUPSTR(posMode, GpsPositionMode));
        }
      }
      ...
      UBX_LOG(LCAT_VERBOSE,
              "SUPL Set Initiated Action started (mode: %s)? %s",
              _LOOKUPSTR(posMode, GpsPositionMode),
              (suplStarted ? "Yes" : "No"));
    }
...
{
#ifdef SUPL_ENABLED
  if ((pState->clientCount > 0) && (pState->clientCount == suplCountSessions(true)))
  {
...
static bool connectReceiver(const ControlThreadInfo *pState, fd_set &rfds, int &rMaxFd)
{
  ...
  if (!s_ser.openSerial(
        pUbxGps->getSerialDevice(), pUbxGps->getBaudRateDefault(), 1, pUbxGps->getI2cTxReady()))
  {
...
static bool
checkRecvInitReq(const ControlThreadInfo *pState, fd_set &rfds, int &maxFd, int64_t timeSinceMsgMs)
{
  CUbxGpsState *pUbxGps = CUbxGpsState::getInstance();
  ...
  else if (pState->gpsState == GPS_STARTED && timeSinceMsgMs > validMsgThresholdMs)
  {
    // we have not received usefull messages within the required interval and
    // need to reinit the receiver
    connectRecv = true;
    UBX_LOG(LCAT_VERBOSE,
            "No valid messages received in the last %lums . "
            "Reinitialising receiver - (Threshold: %lums)",
...
void ubx_thread(void *pThreadData)
{
  ...
  CParserBuffer parser; // declare after protocols, so destructor called before
                        // protocol destructors
  ...
  for (;;)
  {
    ...
    updateTimeLastValid = checkRecvInitReq(
      pState, rfds, maxFd, sinceLastNmeaMs > sinceLastUbxMs ? sinceLastNmeaMs : sinceLastUbxMs);
    ...
    if (res < 0)
    {
      UBX_LOG(LCAT_ERROR, "rselect returned %i. Sleeping for %i second", res, tv.tv_sec);
      sleep(tv.tv_sec);
    }
        ...
        if (availableBytes > 0)
        {
          ...
          if (tmpBuf)
          {
            ...
            if (len > 0)
            {
              ...
              {
                UBX_LOG(
                  LCAT_ERROR, "Unable to write TCP client data of length %i to receiver", len);
              }
            }
            free(tmpBuf);
          }
        ...
        if (iSize)
        {
          ...
          while (parser.Parse(pProtocol, pMsg, iMsg))
          {
            ...
            if (pProtocol == &protocolUBX)
            {
              ...
              std::string message;
              message = byte_array_to_hex_string(static_cast<unsigned char const *>(pMsg), iMsg);
              if (message.compare("") == 0)
              {
                UBX_LOG(LCAT_ERROR,
                        "An error occured while trying to log an "
                        "incoming UBX message");
              }
              else
              {
                UBX_LOG(LCAT_VERBOSE, "MSG UBX RX (size %d): %s", message.size, message.c_str());
              }
            ...
            else
            {
              ...
              {
                UBX_LOG(
                  LCAT_VERBOSE, "MSG UNKNOWN RX (size %d): %s", message.size, message.c_str());
              }
...
#ifdef SUPL_ENABLED
    suplCheckPendingActions(); // Check for any actions on existing SUPL
                               // sessions
#endif                         // ifdef SUPL_ENABLED
...
#ifdef SUPL_ENABLED
///////////////////////////////////////////////////////////////////////////////
//! Implimentation of a requestStart interface function on
...
///////////////////////////////////////////////////////////////////////////////
//! Implimentation of a requestStop interface function on
//! GpsControlEventInterface
/*!
  \param pContext	: Pointer to the interface context
*/
