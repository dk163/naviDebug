...
///////////////////////////////////////////////////////////////////////////////
CAgpsIf::CAgpsIf() : m_callbacks{}, m_agpsServers{}
{
  m_ready = false;
  m_agpsServersCnt = 0;
  ...
  setCertificateFileName(cfg.get("SUPL_CACERT", (const char *)nullptr));
...
CAgpsIf *CAgpsIf::getInstance() { return &s_myIf; }
...
{
  UBX_LOG(
    LCAT_DEBUG, "type=%i(%s) host=%s port=%i", type, _LOOKUPSTR(type, AGpsType), hostname, port);
...
const char *CAgpsIf::getCertificateFileName(void) const { return m_certificateFileName; }

bool CAgpsIf::isTlsActive(void) const { return (m_certificateFileName != NULL); }

int CAgpsIf::getConnectRetries(void) const { return m_connectRetries; };

void CAgpsIf::setConnectRetries(int connectRetries) { m_connectRetries = connectRetries; };
