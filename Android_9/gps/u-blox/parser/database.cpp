...
/*! The first message of an epoch can be defined with this function. Default is
  MSG_NMEA_RMC
...
/*! The first message of an epoch can be defined with this function. Default is
  MSG_NMEA_RMC.
    Be aware that if a message is received that maches 'end', the epoch is
  considered
    as finished. Never select a message as the 'end' message which could appear
  more
    than once within an epoch. The default value for this is MSG_NUM, which
  indicates
    that the commit of the database will happen on receiving the first message
  of the new
    epoch.
  \param end : The message which will be expected on the start of an epoch
*/
void CDatabase::setEndEpoch(MSG_t end) { m_epochEnd = end; }

//! Process the message received with the function provided by the parser
/*! The message of type 'type' found in 'pBuffer' will be parsed by 'func' if
  the epoch
    is considered as having started. The function will also indicate an epoch as
  having
    started or ended based on 'type'. If this function is called to parse a
  message
...
*/
int CDatabase::Process(MSG_t type, const unsigned char *pBuffer, int iSize, PARSE_FUNC_t func)
{
...
CDatabase::STATE_t CDatabase::Commit(bool /*timeBasedEpoch*/)
{
  ...
  if (m_varO[DATA_ERROR_RADIUS_METERS].IsEmpty() && m_varO[DATA_LONGITUDE_DEGREES].IsSet() &&
      m_varO[DATA_LATITUDE_DEGREES].IsSet())
  {
    ...
    else if (m_varO[DATA_SATELLITES_USED_COUNT].Get(i))
    {
      ...
      m_varO[DATA_ERROR_RADIUS_METERS].Set((i >= 6) ? 5 : (i >= 4) ? 10 : 100);
    }
  }
      ...
      m_varO[DATA_ERROR_RADIUS_METERS].IsSet()) //  && m_varO[DATA_LOCAL_TIMESTAMP].IsSet())
  {
    ...
    m_varO[DATA_LASTGOOD_TIMESTAMP] =
      m_varO[DATA_COMMIT_TIMESTAMP]; // m_varO[DATA_LOCAL_TIMESTAMP];
    // optional fields
    m_varO[DATA_LASTGOOD_ALTITUDE_ELLIPSOID_METERS] = m_varO[DATA_ALTITUDE_ELLIPSOID_METERS];
    m_varO[DATA_LASTGOOD_ALTITUDE_ELLIPSOID_ERROR_METERS] =
      m_varO[DATA_ALTITUDE_ELLIPSOID_ERROR_METERS];
    // set state to ready
    m_vasS = STATE_READY;
  }
...
{
  double X, Y;
  if (m_varN[DATA_UBX_POSITION_ECEF_X].Get(X) && m_varN[DATA_UBX_POSITION_ECEF_Y].Get(Y))
  {
    double Z;
...
{
  double X, Y, Long;
  if (m_varN[DATA_UBX_VELOCITY_ECEF_VX].Get(X) && m_varN[DATA_UBX_VELOCITY_ECEF_VY].Get(Y) &&
      m_varN[DATA_LONGITUDE_DEGREES].Get(Long))
  {
    ...
    double Z, Lat;
    if (m_varN[DATA_UBX_VELOCITY_ECEF_VZ].Get(Z) && m_varN[DATA_LATITUDE_DEGREES].Get(Lat))
    {
...
{
  int i;
  // Printf("pc "); DUMP_TS(DATA_LOCAL_TIMESTAMP);
  // DUMP_TS(DATA_LOCALX_TIMESTAMP); Printf("\n");
  ...
  {
    Printf("commit ");
    DUMP_TS(DATA_COMMIT_TIMESTAMP);
    Printf("\n");
    Printf("last good ");
    DUMP_TS(DATA_LASTGOOD_TIMESTAMP);
    Printf("\n");
