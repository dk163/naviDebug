...
static int DEBUG_SV_COUNT = 0;//[ADD-BEGIN] by kang
  ...
  for (int i = 0; i < perMsgData.numCh && ixInView <= CDatabase::MAX_SATELLITES_IN_VIEW; i++)
  {
    ...
    if (validSvId)
    {
      ...
      pDatabase->Set(DATA_UBX_SATELLITES_IN_VIEW_NAV_STA_(ixInView), navsta);
        ...
        //[ADD-BEGIN] by kang
        if((DEBUG_SV_COUNT % 400) == 0)//print sv log
        {
            UBX_LOG(LCAT_VERBOSE, "DEBUG_SV_COUNT = (%d) ", DEBUG_SV_COUNT);
            UBX_LOG(LCAT_VERBOSE, "view SV count = (%d) , navsta = (%d)", ixInView, navsta);
            UBX_LOG(LCAT_VERBOSE, "valid SV count = (%d) ", ixUsed);

            DEBUG_SV_COUNT = 0;
            UBX_LOG(LCAT_VERBOSE, "set DEBUG_SV_COUNT = (%d) ", DEBUG_SV_COUNT);
        }
        DEBUG_SV_COUNT++;
        //[ADD-END]
