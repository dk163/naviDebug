...
int CProtocolNMEA::Parse(const unsigned char *pBuffer, int iSize)
{
  ...
  for (int i = 1; (i < iSize); i++)
  {
    ...
    else if (pBuffer[i] == '\n')
    {
      ...
      {
        // the checksum consists of two hex digits (usually in upper case, but
        // we are tolerant)
        unsigned char highNibble = pBuffer[iAsterix + 1];
        unsigned char lowNibble = pBuffer[iAsterix + 2];
        highNibble = (highNibble >= 'A' && highNibble <= 'F') ?
                       highNibble - 'A' + 10 :
                       (highNibble >= 'a' && highNibble <= 'f') ?
                       highNibble - 'a' + 10 :
                       (highNibble >= '0' && highNibble <= '9') ? highNibble - '0' : 0xFF;
        lowNibble = (lowNibble >= 'A' && lowNibble <= 'F') ?
                      lowNibble - 'A' + 10 :
                      (lowNibble >= 'a' && lowNibble <= 'f') ?
                      lowNibble - 'a' + 10 :
...
void CProtocolNMEA::Process(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase)
{
  if (IsValidNmeaGnssType(pBuffer[2]))
  {
    if (strncmp((char *)&pBuffer[3], "GBS", 3) == 0)
      pDatabase->Process(CDatabase::MSG_NMEA_GBS, pBuffer, iSize, ProcessGBS);
    else if (strncmp((char *)&pBuffer[3], "GGA", 3) == 0)
      pDatabase->Process(CDatabase::MSG_NMEA_GGA, pBuffer, iSize, ProcessGGA);
    else if (strncmp((char *)&pBuffer[3], "GLL", 3) == 0)
      pDatabase->Process(CDatabase::MSG_NMEA_GLL, pBuffer, iSize, ProcessGLL);
    else if (strncmp((char *)&pBuffer[3], "GNS", 3) == 0)
      pDatabase->Process(CDatabase::MSG_NMEA_GNS, pBuffer, iSize, ProcessGNS);
    else if (strncmp((char *)&pBuffer[3], "GRS", 3) == 0)
      pDatabase->Process(CDatabase::MSG_NMEA_GRS, pBuffer, iSize, ProcessGRS);
    else if (strncmp((char *)&pBuffer[3], "GSA", 3) == 0)
      pDatabase->Process(CDatabase::MSG_NMEA_GSA, pBuffer, iSize, ProcessGSA);
    else if (strncmp((char *)&pBuffer[3], "GST", 3) == 0)
      pDatabase->Process(CDatabase::MSG_NMEA_GST, pBuffer, iSize, ProcessGST);
    else if (strncmp((char *)&pBuffer[3], "RMC", 3) == 0)
      pDatabase->Process(CDatabase::MSG_NMEA_RMC, pBuffer, iSize, ProcessRMC);
    else if (strncmp((char *)&pBuffer[3], "VTG", 3) == 0)
      pDatabase->Process(CDatabase::MSG_NMEA_VTG, pBuffer, iSize, ProcessVTG);
    else if (strncmp((char *)&pBuffer[3], "ZDA", 3) == 0)
      pDatabase->Process(CDatabase::MSG_NMEA_ZDA, pBuffer, iSize, ProcessZDA);
  }
...
void CProtocolNMEA::ProcessGBS(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase)
{
  // Time
  CheckSetTime(pBuffer, iSize, pDatabase, 1);
}

void CProtocolNMEA::ProcessGGA(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase)
{
  double d;
  int i;
  char ch;
  ...
  if (GetItem(9, pBuffer, iSize, d) && GetItem(10, pBuffer, iSize, ch) && (toupper(ch) == 'M'))
  {
    ...
    // pDatabase->Set(CDatabase::DATA_ALTITUDE_ANTENNA_SEALEVEL_METERS, d); //
    // todo
  }
...
void CProtocolNMEA::ProcessGLL(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase)
{
  // Time
...
void CProtocolNMEA::ProcessGNS(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase)
{
  double d;
  int i;
  // Time
...
void CProtocolNMEA::ProcessGRS(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase)
{
  // Time
  CheckSetTime(pBuffer, iSize, pDatabase, 1);
  // Mode
...
void CProtocolNMEA::ProcessGSA(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase)
{
...
void CProtocolNMEA::ProcessGST(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase)
{
  double d, e;
  // Time
  CheckSetTime(pBuffer, iSize, pDatabase, 1);
  ...
  // Orient
  // Std Dev Lat / Lon
  if (GetItem(6, pBuffer, iSize, d) && (d < stdDevLimit) && GetItem(7, pBuffer, iSize, e) &&
      (d < stdDevLimit))
  {
    d = sqrt(d * d + e * e);
...
void CProtocolNMEA::ProcessRMC(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase)
{
  char ch;
  double d;
  ...
  // COG Mag
  if (GetItem(10, pBuffer, iSize, d) && GetItem(11, pBuffer, iSize, ch) && (d >= 0.0) &&
      (d <= 180.0))
  {
...
void CProtocolNMEA::ProcessVTG(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase)
{
  double d;
  char ch;
  // COG (true)
  ...
  // SOG (knots)
  if (GetItem(5, pBuffer, iSize, d) && (d >= 0.0) && GetItem(6, pBuffer, iSize, ch) &&
      (toupper(ch) == 'N'))
  {
    pDatabase->Set(CDatabase::DATA_SPEED_KNOTS, d);
  }
  // SOG (km/hr)
  else if (GetItem(7, pBuffer, iSize, d) && (d >= 0.0) && GetItem(8, pBuffer, iSize, ch) &&
           (toupper(ch) == 'K'))
  {
...
void CProtocolNMEA::ProcessZDA(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase)
{
  int year = 0, month = 0, day = 0;
  CheckSetTime(pBuffer, iSize, pDatabase, 1);
  bool bDateOk = GetItem(2, pBuffer, iSize, day) && GetItem(3, pBuffer, iSize, month) &&
...
void CProtocolNMEA::SetLatLon(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase, int ix)
{
  double d;
  char ch;
...
void CProtocolNMEA::SetStatus(const unsigned char *pBuffer, int iSize, CDatabase *pDatabase, int ix)
{
  int i;
  char ch;
  // NMEA: RMC/GLL status
...
void CProtocolNMEA::SetModeIndicator(const unsigned char *pBuffer,
                                     int iSize,
                                     CDatabase *pDatabase,
                                     int ix)
{
...
void CProtocolNMEA::CheckSetTime(const unsigned char *pBuffer,
                                 int iSize,
                                 CDatabase *pDatabase,
                                 int ix)
{
...
int CProtocolNMEA::GetItemCount(const unsigned char *pBuffer, int iSize)
{
...
/*! Return a pointer to the first character of the field iIndex in the NMEA
  string starting
    with pStart and ending with pEnd-1
  \param iIndex     : Index of the field in the NMEA string of whcih the
  position should be found
  \param pBuffer    : Pointer to the NMEA string which should be parsed
  \param pEnd       : Pointer to the character after the last character that
  belongs to the NMEA string
  \return           : On success, the pointer to the first character of the NMEA
  field. If not found, NULL
 */
char *CProtocolNMEA::FindPos(int iIndex, char *pStart, const char *pEnd)
{
  ...
  if ((iIndex == 0) && (pStart < pEnd) && (*pStart != ',') && (*pStart != '*') &&
      (*pStart != '\r') && (*pStart != '\n'))
    return pStart;
  else
    return NULL;
...
/*! Get an NMEA field at position iIndex in pBuffer and return the result as
  double
  \param iIndex     : Index of the field in the NMEA string that should be
  extracted
  \param pBuffer    : Pointer to the NMEA string which should be parsed
  \param iSize      : The number of characters in the NMEA string
  \param dValue     : If successful, the resulting double will be saved in this
  variable
  \return           : true if the field was non-emtpy and could be converted to
  the correct type
 */
bool CProtocolNMEA::GetItem(int iIndex, const unsigned char *pBuffer, int iSize, double &dValue)
{
  char *pEnd = (char *)&pBuffer[iSize];
  ...
  // the string is zero terminated, this ensures correct behaviour of the
  // function
  char chEnd = pEnd[-1];
  pEnd[-1] = '\0';
...
/*! Get an NMEA field at position iIndex in pBuffer and return the result as
  ...
  \param pBuffer    : Pointer to the NMEA string which should be parsed
  \param iSize      : The number of characters in the NMEA string
  \param iValue     : If successful, the resulting integer will be saved in this
  variable
  \param iBase      : The base that is used for the integer conversion (0 will
  distinguish
                      between 8, 10, 16 depending on the prefix automatically)
  \return           : true if the field was non-emtpy and could be converted to
  the correct type
 */
bool CProtocolNMEA::GetItem(
  int iIndex, const unsigned char *pBuffer, int iSize, int &iValue, int iBase /*=10*/)
{
  char *pEnd = (char *)&pBuffer[iSize];
  char *pPos = FindPos(iIndex, (char *)pBuffer, pEnd);
  // find the start
...
/*! Get an NMEA field at position iIndex in pBuffer and return the first
  character
  \param iIndex     : Index of the field in the NMEA string that should be
  extracted
  \param pBuffer    : Pointer to the NMEA string which should be parsed
  \param iSize      : The number of characters in the NMEA string
  \param chValue    : If successful, the resulting double will be saved in this
  variable
  \return           : true if the field was non-emtpy and could be converted to
  the correct type
 */
bool CProtocolNMEA::GetItem(int iIndex, const unsigned char *pBuffer, int iSize, char &chValue)
{
  char *pEnd = (char *)&pBuffer[iSize];
  char *pPos = FindPos(iIndex, (char *)pBuffer, pEnd);
  // find the start
  ...
  // check bound
  if ((pPos < pEnd) && (*pPos != ',') && (*pPos != '*') && (*pPos != '\r') && (*pPos != '\n'))
  {
    chValue = *pPos;
    return true;
  }
