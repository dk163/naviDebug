...
CThreadedPacketIf::CThreadedPacketIf()
  : _com(NULL), _isOpen(false), _fd_packet_usr(-1), _fd_packet_thread(-1)
{
}
...
/*! The return value will indicate if the \ref open
    ...
    \return            Will return true if the function
                       ...
                       otherwise.
*/
bool CThreadedPacketIf::isOpen() { return _isOpen; }
...
*/
size_t
CThreadedPacketIf::process(void const *context, size_t source, unsigned char *buf, size_t size)
{
  assert(context);
  assert(buf);
  assert(size);
...
{
  assert(context);
  ((CThreadedPacketIf *)const_cast<void *>(context))->errorOccurred(operation, fdindex, correrrno);
}
