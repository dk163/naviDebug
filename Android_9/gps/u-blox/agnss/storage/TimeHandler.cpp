...
*/
CTimeHandler::CTimeHandler(CLockedStorage &storage) : _storage(storage) {}
...
bool CTimeHandler::getCurrentTime(ACCTIME_t *accTime)
{
  ...
  if (getInjectedTime(&injAccTime, &stampDiff))
  {
    ...
    accTime->acc.tv_sec = injAccTime.acc.tv_sec + stampDiff.tv_sec / INACC_TIME_RATIO;
...
bool CTimeHandler::isValid(STAMPED_TIME_t const *stTime)
{
  ...
  bool result = false;
  struct timespec now;
  if (getMonotonicCounter(&now) && timecmp(&now, &stTime->stamp) > 0) // Mon. now later than stored?
    result = true;
