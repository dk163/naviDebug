...
/**
        ...
        \note The bits that are to be extract must not span more than two data
   words.
...
#define GPSSFRU4DW(pkAdr, ofs, num)                                                                \
  ((U4)(((pkAdr[(ofs) / 30] >> 6) << (((ofs) % 30) + 8)) |                                         \
        ((24 >= ((ofs) % 30 + (num))) ? 0 : (((U4)(pkAdr)[(ofs) / 30 + 1] << 2) >>                 \
                                             (24 - ((ofs) % 30))))) >>                             \
   (32 - (num)))
...
/**
        ...
        \note The bits that are to be extract must not span more than one data
   word.
...
#define GPSSFRU4SW(pkAdr, ofs, num)                                                                \
  ((U4)((pkAdr[(ofs) / 30] >> 6) << (((ofs) % 30) + 8)) >> (32 - (num)))
...
#define GPSSFRI4SW(pkAdr, ofs, num)                                                                \
  ((I4)((pkAdr[(ofs) / 30] >> 6) << (((ofs) % 30) + 8)) >> (32 - (num)))
...
/**
        ...
        \note The bits that are to be packed must not span more than one data
   word
...
#define GPSSFRSW_R(pkAdr, ofs, num, word)                                                          \
  pkAdr[(ofs) / 30] = (pkAdr[(ofs) / 30] & ~(MASK(num) << (30 - ((ofs) % 30) - (num)))) |          \
...
#define GPSSFRDW_R(pkAdr, ofs, num, word)                                                          \
  ...
  GPSSFRSW_R(pkAdr, (ofs) + (30 - ((ofs) % 30)), (num) - (24 - MIN(24, (ofs) % 30)), word);
