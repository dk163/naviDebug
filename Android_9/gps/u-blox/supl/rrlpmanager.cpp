...
//#define TIME_REFERENCE_OFFSET_MILLS  1500		/*!< offset for
//assumption how old is the time reference: now it seems 1500 in average! */
#define TIME_REFERENCE_OFFSET_MILLS                                                                \
  0 /*!< offset for assumption how old is the time reference: now it seems     \
       1500 in average! */
#define GPS_TIME_REFERENCE_TOLERANCE                                                               \
  2500 //!< the tolerance in setting the time reference should be less then 3                      \
       //!seconds
...
#define _LOOKUPSTR(v, t) _strLookup(v, str##t)
  ...
  std::vector<std::string> strRrlpMessageType = {
    "NOTHING",           //
    "msrPositionReq",    //
    "msrPositionRsp",    //
    "assistanceData",    //
  ...
  std::vector<std::string> strPositionMethodType = { "eotd", "gps", "gpsOrEOTD" };

  std::vector<std::string> strMethodTypeType = {
    "NOTHING",       //
    "msAssisted",    //
    "msBased",       //
    "msBasedPref",   //
  ...
  // lambda to assert value during testing - refactored from existing code
  auto assertTrue = [](bool const v) {
    assert(v == true);
    // if assert is not defined just log message
    if (v != true)
    {
      UBX_LOG(LCAT_ERROR, "Value check failed in %s Line %d ", __FILE__, __LINE__);
    }
  };
  auto assertZero = [](bool const v) {
    assert(v == 0);
    // if assert is not defined just log message
    if (v != 0)
    {
...
static void
convertNavModelForSat(int satellite, const UncompressedEphemeris_t *pemer, int tow, int wn);
...
/*! Function for decoding and processing RRLP messages
  ...
  \param pOutSize     : size of the response payload to be returned to the
  server
  \param pAux         : Auxiliary parameter OUT, for RRLP responce management
  \return             pointer to RRLP response message to be returned to the
  server
*/
  ...
  *pOutSize = 0;
  ...
  if (rrlpMsg->component.present == RRLP_Component_PR_assistanceData)
  {
    ...
    if (rrlpMsg->component.choice.assistanceData.gps_AssistData != NULL)
    {
      ...
      //			pAux->assistDataReceived = true;
      manageAssistanceData(
        sid, &(rrlpMsg->component.choice.assistanceData.gps_AssistData->controlHeader));
    }
    else
    {
  ...
  else if (rrlpMsg->component.present == RRLP_Component_PR_msrPositionReq)
  {
    ...
    if (rrlpMsg->component.choice.msrPositionReq.gps_AssistData != NULL)
    {
      ...
      //			pAux->assistDataReceived = true;
      manageAssistanceData(
        sid, &(rrlpMsg->component.choice.msrPositionReq.gps_AssistData->controlHeader));
    }

    UBX_LOG(LCAT_VERBOSE,
            "PositionInstruct: PosMethod (%s)  :  PosType (%s)",
            _LOOKUPSTR(rrlpMsg->component.choice.msrPositionReq.positionInstruct.positionMethod,
                       PositionMethodType),
            // lint -e{732}
            _LOOKUPSTR(rrlpMsg->component.choice.msrPositionReq.positionInstruct.methodType.present,
                       MethodTypeType));

    // Now check the positioning instruction */
    if ((rrlpMsg->component.choice.msrPositionReq.positionInstruct.methodType.present ==
         ...
         MethodType_PR_msBasedPref))
    {
      ...
      int accuracy =
        rrlpMsg->component.choice.msrPositionReq.positionInstruct.methodType.choice.msBased;
      ...
      pAux->responseTime =
        1 << rrlpMsg->component.choice.msrPositionReq.positionInstruct.measureResponseTime;
    ...
    else if ((rrlpMsg->component.choice.msrPositionReq.positionInstruct.methodType.present ==
              ...
              MethodType_PR_msAssistedPref))
    {
      ...
      UBX_LOG(LCAT_VERBOSE,
              "MS-ASSIST responce time : %d(s) ref num : %d",
              pAux->responseTime,
    ...
    else
    {
      // Wrong method
      UBX_LOG(LCAT_VERBOSE,
              "Unknown/wrong positional request method (%i). Not supported",
              rrlpMsg->component.choice.msrPositionReq.positionInstruct.methodType.present);

      // Calling function will tidy up and send protocol error because no
      // response msg is returned
  ...
  else
  {
    ...
    UBX_LOG(
      LCAT_VERBOSE, "Unknown/wrong RRLP method (%i). Not supported", rrlpMsg->component.present);

    // Calling function will tidy up and send protocol error because no response
    // msg is returned
    // and the deferred response time (pAux->responseTime) is 0
  }
...
/*! Function for building an RRLP position response message due to a timeout
  \param pOutSize         : size of the encoded RRLP response message to be
  returned
  \param referenceNumber  : RRLP req reference number to use in the response
  message
  \return   Pointer to the encoded RRLP response message to be returned to the
  server
*/
...
char *buildRrlpMsaPosResp(int sid, int *pOutSize, int referenceNumber)
{
  ...
  assertTrue(pDatabase->GetOutput(CDatabase::DATA_UBX_SATELLITES_IN_MEAS_COUNT, svCount));
  if (svCount > 0)
  {
    // Only add optional GPS_MeasureInfo as we have 1 or more satellites
    msg.component.choice.msrPositionRsp.gps_MeasureInfo =
      (GPS_MeasureInfo_t *)MC_CALLOC(sizeof(GPS_MeasureInfo_t), 1);

    I4 gnssTow = 0;
    assertTrue(pDatabase->GetOutput(CDatabase::DATA_UBX_GNSS_TOW, gnssTow));

    I4 dopCenter = 0;
    assertTrue(pDatabase->GetOutput(CDatabase::DATA_UBX_GNSS_DOP_CENTER, dopCenter));
    UBX_LOG(LCAT_VERBOSE, "dop Center %i (Hz) ", dopCenter);
    dopCenter *= 5; // Scale to 0.2 resolution

    GPS_MsrSetElement *pMsrSetElement =
      (GPS_MsrSetElement *)MC_CALLOC(sizeof(GPS_MsrSetElement), 1);
    assert(pMsrSetElement);
    if (pMsrSetElement == nullptr)
    {
      //Try to free anyways
    ...
    UBX_LOG(LCAT_VERBOSE,
            "svCount=%i gnssTow=%i GPSTOW24b_t=%li",
            svCount,
            gnssTow,
            pMsrSetElement->gpsTOW);
    ...
    {
      I4 svid = 0;
      assertTrue(pDatabase->GetOutput(DATA_UBX_SATELLITES_IN_MEAS_(i), svid));

      I4 cNo = 0;
      assertTrue(pDatabase->GetOutput(DATA_UBX_SATELLITES_IN_MEAS_CNO_(i), cNo));

      iBuf += sprintf(&buf[iBuf], " ,%d,%d", svid, cNo);

      R8 prRms = 0;
      assertTrue(pDatabase->GetOutput(DATA_UBX_SATELLITES_IN_MEAS_PRRMS_(i), prRms));

      U4 multipathIndicator = 0;
      assertTrue(
        pDatabase->GetOutput(DATA_UBX_SATELLITES_IN_MEAS_MULTIPATH_IND_(i), multipathIndicator));

      I4 redSigtow = 0;
      assertTrue(pDatabase->GetOutput(DATA_UBX_SATELLITES_IN_MEAS_REL_CODEPHASE_(i), redSigtow));

      I4 doppler = 0;
      assertTrue(pDatabase->GetOutput(DATA_UBX_SATELLITES_IN_MEAS_DOPPLER_(i), doppler));
      ...
      if ((adjustedDoppler >= -32768) && (adjustedDoppler <= 32767))
      {
        ...
        if (redSigtow > 0)
        {
          tmp2 = ((R8)redSigtow / (R8)0x200000); // Calculate the floating point
                                                 // version of the pseudorange
                                                 // codephase
          tmp2 = tmp2 - floor(tmp2);             // Remove the integer part (it remains only
                                                 // the millisecond fraction)
          tmp2 = 1 - tmp2; // Here we have to consider the residual part of the
                           // millisecond.
        }
        else
        {
          tmp2 = -((R8)redSigtow / (R8)0x200000); // Calculate the floating
        ...
        pMsrElement->fracChips =
          (long)((tmp2 - floor(tmp2)) * 1024.); // fraction of a chip, in 10 bits representation
        // UBX_LOG(LCAT_VERBOSE, "new phase %li %li (redSigtow=%d tmp=%lf)",
        // pMsrElement->wholeChips, pMsrElement->fracChips, redSigtow, tmp2);
        ...
        UBX_LOG(
          LCAT_VERBOSE,
          "MSA Data - sv G%3ld : CNO %2ld : "
          "Doppler(-dopCenter) %7.1f : Chips %9.4f : MPI "
          "%ld : PRE %ld",
          pMsrElement->satelliteID + 1,
          pMsrElement->cNo,
          (double)pMsrElement->doppler / 5.0,
          (((double)pMsrElement->wholeChips * (double)0x400) + (double)(pMsrElement->fracChips)) /
            (double)0x400,
          pMsrElement->mpathIndic,
          pMsrElement->pseuRangeRMSErr);

        assertZero(ASN_SEQUENCE_ADD(&pMsrSetElement->gps_msrList, pMsrElement));
        // lint -e{429} removed Custodial pointer 'pMsrElement' (line 386) has
    ...
    sprintf(&buf[iBuf], " #measurement Info");
    logAgps.write(0x00000004, buf);

    if (validSv)
    {
      // Add list of valid satellite info
      assertZero(ASN_SEQUENCE_ADD(
        &msg.component.choice.msrPositionRsp.gps_MeasureInfo->gpsMsrSetList, pMsrSetElement));
    }
    else
    {
      // Need to release the sv info manually and tidy up, as we won't be
      // sending it to the server
      MC_FREE(msg.component.choice.msrPositionRsp.gps_MeasureInfo);
      msg.component.choice.msrPositionRsp.gps_MeasureInfo = NULL;
      MC_FREE(pMsrSetElement);
    }
    // lint -e{593} removed Custodial pointer 'pMsrSetElement' (line 342)
    // possibly not freed or returned
  }
...
/*! Function for building an RRLP position response message containing a
  position
  \param pOutSize         : size of the encoded RRLP response message to be
  returned
  \param referenceNumber  : RRLP req reference number to use in the response
...
*/
char *
buildRrlpPosResp(int *pOutSize, int referenceNumber, double lat, double lon, double alt, double tow)
{
  PDU_t msg{};
  char *buf = NULL;
  ((void)(tow));
  ...
  msg.component.choice.msrPositionRsp.locationInfo =
    (LocationInfo_t *)MC_CALLOC(sizeof(LocationInfo_t), 1);
  ...
  //	msg.component.choice.msrPositionRsp.locationInfo->gpsTOW = (long *)
  //MC_CALLOC(sizeof(long), 1);
  ...
  if (lat < 0)
  {
    msg.component.choice.msrPositionRsp.locationInfo->posEstimate.buf[1] |=
      0x80; // -ve : Set sign bit
  }
  ...
  if (alt < 0)
  {
    msg.component.choice.msrPositionRsp.locationInfo->posEstimate.buf[7] |=
      0x80; // -ve : Set sign bit
  }
...
/*! Pack ephemeris data into ephemeris sub-frame data format
        \param pkEph    : Pointer to ephemeris data conatined in a general
   structure
...
*/
static void
convertNavModelForSat(int satellite, const UncompressedEphemeris_t *pEphemer, int tow, int wn)
{
  NAV_GPS_EPH_COLLECT_t eph{};

  // According to IS-GPS-200, a broadcast ephemeris may have a maximum fit
  // interval of 98 hours, roughly +/- 2 days
  // Thus a valid ephemeris which is parametrized in the subframe format as
  // defined by IS-GPS-200 cannot be older than 2 days.
...
/*! Converts data in the Supl/RRLP Utc model to the equivalent Ubx message
   format
...
*/
static void fillIonosphericModel(GPS_UBX_AID_HUI_t *pUbxUtcModel,
                                 const IonosphericModel_t *pIonoModel)
{
...
static void manageAssistanceData(int sid, const ControlHeader_t *pCtl)
{
  ...
  {
    /* Fill the data with the information coming from the RRLP message */
    fillRefLoc(&hdl, pCtl->refLocation->threeDLocation.buf, pCtl->refLocation->threeDLocation.size);
    aidingPresent = true;
  }
  ...
  {
    /* Navigation model assisted data is present ! (and the time reference has
     * been received) */
    int i;
    UBX_LOG(LCAT_VERBOSE,
            "Navigation model assisted data is present for %d satellites",
            pCtl->navigationModel->navModelList.list.count);

    char buf[256];
    int iBuf = sprintf(buf,
                       "%d, 1.0.0, %d, %d, %d",
                       sid,
                       pCtl->navigationModel->navModelList.list.count,
                       towe,
    ...
    for (i = 0; i < pCtl->navigationModel->navModelList.list.count; i++)
    {
      ...
      else
      {
        ...
        UBX_LOG(
          LCAT_VERBOSE, "Navigation Model found for satellite G%ld", pElement->satelliteID + 1);
    ...
    sprintf(&buf[iBuf], " # ephemeris info");
    logAgps.write(0x00000004, buf);
  }
...
/*! Converts a current reference time for use in Ubx message format
    ...
    the earliest sane time \ref SRC_CREAT_TIME_SEC
        \param pHandl : Pointer to Ubx message structure containing reference
   time
...
static void fillRefTime(GPS_UBX_AID_INI_U5__t *pHandl, U4 tow, U2 wn)
{
  ...
  pHandl->tow = t.tow.tv_sec * MSEC_PER_SEC + t.tow.tv_nsec / NSEC_PER_MSEC;
}
...
static void fillRefLoc(GPS_UBX_AID_INI_U5__t *pHandl, const unsigned char *pBuf, int size)
{
  ...
  if (((type == 0x00) && (size != 7)) || ((type == 0x01) && (size != 8)) ||
      ((type == 0x03) && (size != 11)) || ((type == 0x08) && (size != 9)) ||
      ((type == 0x09) && (size != 14)))
  {
    /* size checks */
    writeOk = 0;
  ...
  case 0x00:
    /* ellipsoid point */
    /* set the flag */
    pHandl->flags |= (GPS_UBX_AID_INI_U5__FLAGS_POS_MASK | GPS_UBX_AID_INI_U5__FLAGS_LLA_MASK |
  ...
  if (writeOk)
  {
    ...
    latitude = convertLatitude(((pBuf[1] & 0x7F) << 16) + (pBuf[2] << 8) + pBuf[3]);

    latitude = (pBuf[1] & 0x80) ? -latitude : latitude;

    longitude = convertLongitude((pBuf[4] << 16) + (pBuf[5] << 8) + pBuf[6]);
